...
- LSP code-intelligence boundary: the next LSP MVP is explicitly read-only and local-first. Allowed surfaces are only `initialize`, `initialized`, `shutdown`, `exit`, bounded `textDocument/didOpen`, `textDocument/didChange`, `textDocument/didClose`, and deterministic local `textDocument/completion` or status proof over editor-supplied in-memory documents. It must not perform workspace edits, file writes, shell/tool execution, apply patches, autonomous indexing, arbitrary file reads, provider-backed completion on keystrokes, production agent behavior, raw secret exposure, raw unbounded document logging, or any hosted Yet AI backend/account/managed gateway/product-credit/cloud-workspace requirement. JetBrains LSP client wiring is deferred for this MVP sprint; the JetBrains shell remains chat/runtime/active-context only until a later card defines and verifies the IntelliJ LSP API path, stdio lifecycle, URI/document bounds, diagnostics, and smoke coverage.
