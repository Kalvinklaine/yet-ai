...
- Provider secret storage status: new provider API keys are stored by the engine-owned secret store in the current protected-file fallback under user config, not in provider config JSON or GUI/browser storage. Legacy provider configs that still contain inline `auth.apiKey` are migrated on normal provider/model/test/chat access: the engine writes the secret first with atomic create-if-absent semantics, then scrubs the provider config. Existing stored secrets win over stale inline values, unsafe secret-store failures return sanitized errors without using inline fallback, and a scrub failure after a successful write can be retried later. This is not production OS keychain support.
