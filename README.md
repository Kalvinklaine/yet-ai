...
- Limitations: the baseline is not production-ready; no marketplace packaging, signed or notarized engine bundles, production installer, autonomous file reads/indexing, production LSP/completions/tools, autonomous file edits/apply patch, shell/tool execution, full agent autonomy, background agent autonomy, production no-idle scheduler, or integration workflows are complete. The confirmed edit-proposal MVP is preview-and-confirm only, VS Code-only on the host apply side, limited to bounded text replacements in existing workspace-relative files, and not production agent autonomy. Tools, tasks, and knowledge remain disabled. Current chat is a local provider/chat MVP only.
...
Run the focused VS Code controlled-action wrapper smoke when changing controlled IDE action copy, GUI fallback rendering, or the VS Code bridge path:
...
`npm run smoke:vscode-wrapper-browser` uses packaged VS Code GUI assets in a browser harness to verify the current VS Code-first controlled action contract without launching real VS Code. It covers assistant-authored strict JSON read-only IDE action proposal rendering, no auto-execution before the user clicks, explicit user confirmation, fresh GUI-owned `gui.ideActionRequest` ids, correlated host progress/result rendering, retained manual `getContextSnapshot` request/result rendering, client-side `Clear pending IDE action state`, stale progress/result ignore after clear, bounded duplicate result handling, and secret redaction. Browser and JetBrains remain preview/unsupported for executing controlled actions in this sprint. `getContextSnapshot` returns active-context metadata only for controlled IDE actions, open/reveal are navigation only, and action result metadata must not include raw file contents. `cargo test -p yet-lsp agent_progress` covers the engine metadata-only progress endpoint. These checks do not execute IDE tools from the engine, mutate workspaces, run shell/git/tasks/tools, read arbitrary files, call providers, or require hosted services.
