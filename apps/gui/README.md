...
Controlled IDE action fallback coverage is VS Code-first for this sprint:
...
Use it when changing GUI controlled-action copy, fallback rendering, or progress/action metadata display. The browser shell can render sanitized progress/result messages in harnesses, but it does not execute controlled IDE actions itself. JetBrains is also preview-only/unsupported for controlled actions in this sprint. These automated smokes are loopback/mock harnesses and are not a substitute for manual VS Code dogfood; the T-547 manual VS Code dogfood was not run.
...
The GUI-to-host bridge sends strict `gui.ready`, strict safe `gui.ideActionRequest` for controlled context/navigation actions, and, for the first confirmed edit-proposal MVP, strict `gui.applyWorkspaceEditRequest`. It may also send strict `gui.unloaded` with an empty payload as a non-privileged iframe/page lifecycle notification; this is not an execution command or write path. The Agent activity / IDE actions panel is exposed as working only in VS Code host mode and can request metadata-only `getContextSnapshot`, plus navigation-only `openWorkspaceFile` / `revealWorkspaceRange` when the current active context has a safe workspace-relative path/range. Browser mode is non-privileged and JetBrains is preview-only/unsupported for these controlled actions, so the GUI does not post privileged IDE action requests there. The GUI can also render the latest complete assistant `assistant.ideActionProposal` as a separate read-only proposal card with sanitized summary/action/path/range and explicit confirmation copy; raw strict JSON assistant proposals are compacted in chat, `Inspect proposal JSON` is on-demand/debug only, and the proposal card remains the explicit confirmation surface. The GUI never auto-runs proposals, ignores assistant-supplied request ids through the parser/schema, and only VS Code shows a `Run read-only IDE action` button that posts a fresh GUI-owned `gui-ide-proposal-action-N` request after the user clicks. Pending IDE action state can be cleared in the GUI with `Clear pending IDE action state`; this is client-side state cleanup only and does not cancel any already-open host-side picker/dialog/UI by itself. IDE action progress/results are correlated by request id and active chat; stale updates after clear, chat switch, or settings/runtime change are ignored. Completed duplicate results are bounded/cleared instead of accumulated, and the panel renders only safe labels, status, workspace-relative paths/ranges, and sanitized summaries/messages without raw payload dumps, raw file contents, or browser-storage persistence.
...
The Agent activity / IDE actions panel is a VS Code-only controlled action surface for safe context/navigation requests. It shows browser unsupported and JetBrains preview-only states outside VS Code, blocks duplicate pending clicks, and never stores action history or secrets in browser storage.
