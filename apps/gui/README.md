...
In VS Code or JetBrains `auto` / `launch` mode, the local runtime session token is normally generated by the trusted IDE host, passed to `yet-lsp` as `YET_AI_AUTH_TOKEN`, and supplied to the GUI through a validated `host.ready` bridge message. Do not paste `local-dev-token` into the GUI in that mode.
...
1. Use IDE `auto` / `launch` mode or start the manual runtime command above.
...
4. Paste the provider API key once in the API key field, save, and confirm the field clears. For local mock providers, use only fake credentials.
5. Send `Say hello in one sentence.` Expected behavior: the command is accepted, SSE opens, and the assistant response streams through snapshot/start/delta/finish events without any Yet AI hosted backend.
