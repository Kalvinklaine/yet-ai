...
5. Use provider test/status as sanitized feedback, then send `Say hello in one sentence.` Expected behavior: the command is accepted, SSE opens, and the assistant response streams through snapshot/start/delta/finish events without any Yet AI hosted backend.
