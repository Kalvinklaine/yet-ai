...
import { buildProjectRoute, navigateProjectRoute } from "../services/projectRouting";
...
export function ProjectHub({ settings }: { settings: RuntimeSettings }) {
  ...
  const refresh = useCallback(async (background = false) => {
    ...
    setError(null);
    const result = await listProjects(settings);
  ...
  return (
    <main className="project-page-shell">
      <header className="project-product-header">
        <a className="product-mark" href="/projects" aria-label="Yet AI projects"><span aria-hidden="true">Y</span><strong>Yet AI</strong></a>
        <a className="project-settings-link" href={buildProjectRoute({ kind: "settings" })}>Settings</a>
      </header>
      ...
      {active.length > 0 && <ProjectList title="Your projects" projects={active} onMutate={mutate} />}
      {legacyAvailable && <section className="legacy-project-entry"><div><span className="badge warn">compatibility</span><h2>Unscoped legacy data</h2><p>Older local data is kept separate from registered projects.</p></div><a href={buildProjectRoute({ kind: "legacy" })}>Open legacy data</a></section>}
      {archived.length > 0 && <ProjectList title="Archived projects" projects={archived} onMutate={mutate} />}
      {dialogOpen && <ProjectRegistrationDialog settings={settings} onClose={closeDialog} onRegistered={(project) => navigateProjectRoute(window, { kind: "project", projectId: project.projectId, page: "home" })} />}
    </main>
  );
}

function ProjectList({ title, projects, onMutate }: { title: string; projects: ProjectSummary[]; onMutate: (project: ProjectSummary, action: "rename" | "archive" | "restore") => void }) {
  return <section className="project-list-section"><div className="project-list-heading"><h2>{title}</h2><span className="subtle">{projects.length} total</span></div><div className="project-table-wrap"><table className="project-table"><thead><tr><th scope="col">Project</th><th scope="col">Readiness</th><th scope="col">Recent activity</th><th scope="col">Last opened</th><th scope="col">Actions</th></tr></thead><tbody>{projects.map((project) => <ProjectRow key={project.projectId} project={project} onMutate={onMutate} />)}</tbody></table></div></section>;
}

function ProjectRow({ project, onMutate }: { project: ProjectSummary; onMutate: (project: ProjectSummary, action: "rename" | "archive" | "restore") => void }) {
