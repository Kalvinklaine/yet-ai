...
export function ControlledAgentFileReadPanel({ metadata }: ControlledAgentFileReadPanelProps) {
  const [open, setOpen] = useState(false);
  const read = useMemo(() => evaluateControlledAgentFileRead(metadata), [metadata]);
      ...
      <details className="debug-details" data-testid="controlled-agent-file-read-details" open={open} onToggle={(event) => setOpen(event.currentTarget.open)}>
        ...
        {open && (
          <div className="stack">
            ...
            <span className="subtle">No action controls are rendered. Future runtime read endpoints must be added explicitly before any read request can be made from the GUI.</span>
          </div>
        )}
      </details>
    </section>
