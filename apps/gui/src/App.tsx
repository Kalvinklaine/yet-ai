...
export function App({ route = { kind: "legacy" }, navigate, runtimeSettings, onRuntimeSettingsChange, bridgeAdapter, hostedAuthorityKey, hostReadyGeneration, acceptedHostReadySeed }: { route?: Exclude<AppRoute, { kind: "not_found" | "projects" }>; navigate?: ProjectNavigation; runtimeSettings?: RuntimeSettings; onRuntimeSettingsChange?: (settings: RuntimeSettings) => void; bridgeAdapter?: BridgeAdapter; hostedAuthorityKey?: string; hostReadyGeneration?: string | null; acceptedHostReadySeed?: AcceptedHostReadySeed | null }) {
  ...
  const controlledCapabilityProvenance = useMemo(() => classifyControlledCapabilityProvenance({
    host: bridgeHost,
    caps: activeCaps,
        ...
        {launchIntentCreateVisible ? (
          ...
          <section className="chat-thread-pane" aria-label="Current chat thread">
            <div className="chat-title-card chat-compact-header row">
              ...
              <span className="badge chat-id-badge">{sanitizeDisplayText(chatId ?? "draft")}</span>
            </div>
              ...
              {(projectPage === undefined || ideActionsAuthorized) && <div className="composer-tools">
                {projectPage === undefined && <details className="composer-tool-drawer" data-testid="task-agent-tools-drawer">
                  ...
                  <div className="composer-drawer-body stack">
                    ...
                    {showControlledAgentRunPanel && <ControlledAgentRunPanel state={controlledAgentRunState} progressReport={controlledAgentProgressReport} mvpReport={controlledLocalAgentMvpReport} host={bridgeHost} capabilityMatrix={controlledHostCapabilities ? controlledHostCapabilityMatrix : undefined} provenance={controlledCapabilityProvenance} twoStepRunState={controlledAgentTwoStepRunMetadata !== undefined ? controlledAgentTwoStepRunState : undefined} onStop={stopControlledAgentRun} />}
                    {controlledWorkspaceReadinessMetadata !== undefined && <ControlledAgentWorkspaceReadinessPanel metadata={controlledWorkspaceReadinessMetadata} provenance={controlledCapabilityProvenance.workspace_readiness} />}
                    {(controlledAgentFileReadMetadata !== undefined || controlledAgentFileReadRequest.state !== "blocked") && <ControlledAgentFileReadPanel provenance={controlledCapabilityProvenance.controlled_read} metadata={effectiveControlledAgentFileReadMetadata} evaluatedRead={controlledAgentFileReadSummary} request={controlledAgentFileReadRequest} pendingRequestId={pendingControlledFileReadRequestId} note={controlledFileReadNote} onRequest={submitControlledFileRead} onClearPending={clearPendingControlledFileReadState} />}
