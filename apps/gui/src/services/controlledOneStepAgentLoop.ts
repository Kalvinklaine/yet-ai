...
export function reduceControlledOneStepAgentLoopState(current: ControlledOneStepAgentLoopState, event: unknown): ControlledOneStepAgentLoopState {
  ...
  if (eventError) return stoppedState("failed", current.budgets, current.counters, eventError, false, stopMessage(eventError), current.diagnostics);
...
function startLoop(current: ControlledOneStepAgentLoopState, metadata: ControlledOneStepAgentLoopStartInput): ControlledOneStepAgentLoopState {
  if (current.phase !== "idle") return stoppedState("failed", current.budgets, current.counters, "invalid_transition", false, stopMessage("invalid_transition"), current.diagnostics);
  ...
  if (diagnostics.length > 0) return stoppedState("failed", budgets, current.counters, "unsafe_metadata", false, stopMessage("unsafe_metadata"), diagnostics);
  if (!isPlainObject(metadata) || metadata.source !== "gui" || metadata.confirmedBy !== "user" || metadata.assistantMinted !== false || metadata.explicitUserStart !== true || !safeId(metadata.requestId)) {
    return stoppedState("failed", budgets, current.counters, "missing_user_start", false, stopMessage("missing_user_start"), current.diagnostics);
  }
  return buildState("start_requested", true, false, budgets, { ...zeroCounters, userTurns: 1 }, safeText(typeof metadata.summary === "string" ? metadata.summary : "Explicit user start recorded for one-step loop.", 240), []);
}

function recordRead(current: ControlledOneStepAgentLoopState, metadata: unknown): ControlledOneStepAgentLoopState {
  if (current.phase !== "start_requested") return stoppedState("failed", current.budgets, current.counters, "invalid_transition", false, stopMessage("invalid_transition"), current.diagnostics);
  const read = evaluateControlledAgentFileRead(metadata);
  if (read.state !== "success" && read.state !== "truncated") return stoppedState("failed", current.budgets, current.counters, "read_blocked", false, "Bounded read evidence was blocked.", diagnosticsFromText(current.diagnostics, read.diagnostics.map((item) => item.message)));
  ...
  return enforceBudgets(buildState("read_context", true, false, current.budgets, counters, read.summary, current.diagnostics));
}
...
function recordModelStep(current: ControlledOneStepAgentLoopState, metadata: ControlledOneStepAgentLoopProposalInput): ControlledOneStepAgentLoopState {
  if (current.phase !== "read_context") return stoppedState("failed", current.budgets, current.counters, "invalid_transition", false, stopMessage("invalid_transition"), current.diagnostics);
  ...
  if (!safe || diagnostics.length > current.diagnostics.length) return stoppedState("failed", current.budgets, current.counters, diagnostics.length > current.diagnostics.length ? "unsafe_metadata" : "unsafe_proposal", false, stopMessage(diagnostics.length > current.diagnostics.length ? "unsafe_metadata" : "unsafe_proposal"), diagnostics);
  const counters = { ...current.counters, loopSteps: current.counters.loopSteps + 1 };
  return enforceBudgets(buildState("model_step_pending", true, false, current.budgets, counters, safeText(typeof metadata.summary === "string" ? metadata.summary : "One sanitized model proposal step recorded.", 240), diagnostics));
}

function recordEdit(current: ControlledOneStepAgentLoopState, metadata: unknown): ControlledOneStepAgentLoopState {
  if (current.phase !== "model_step_pending" && current.phase !== "edit_ready") return stoppedState("failed", current.budgets, current.counters, "invalid_transition", false, stopMessage("invalid_transition"), current.diagnostics);
  ...
  if (edit.diagnostics.length > 0 || (edit.state !== "planned" && edit.state !== "applied")) {
    return stoppedState("failed", current.budgets, current.counters, "edit_failed", true, "Bounded edit evidence failed closed.", diagnosticsFromText(current.diagnostics, edit.diagnostics));
  }
  ...
  return enforceBudgets(buildState(phase, true, false, current.budgets, counters, edit.summary, current.diagnostics));
}

function recordVerification(current: ControlledOneStepAgentLoopState, metadata: unknown): ControlledOneStepAgentLoopState {
  if (current.phase !== "edit_applied" && current.phase !== "verification_requested") return stoppedState("failed", current.budgets, current.counters, "invalid_transition", false, stopMessage("invalid_transition"), current.diagnostics);
  ...
  if (command.state === "succeeded") return enforceBudgets(buildState("completed", false, true, current.budgets, counters, command.summary, current.diagnostics));
  if (command.state === "running") return enforceBudgets(buildState("verification_requested", true, false, current.budgets, counters, command.summary, current.diagnostics));
  const reason = command.state === "timed_out" ? "timeout" : "verification_failed";
  return stoppedState("failed", current.budgets, counters, reason, reason === "verification_failed", command.summary, diagnosticsFromText(current.diagnostics, command.diagnostics.map((item) => item.message)));
}
...
function enforceBudgets(state: ControlledOneStepAgentLoopState): ControlledOneStepAgentLoopState {
  ...
  return reason ? stoppedState("failed", state.budgets, state.counters, reason, false, stopMessage(reason), state.diagnostics) : { ...state, details: buildDetails(state.budgets, state.counters) };
}
...
function stoppedState(phase: "failed" | "stopped", budgets: ControlledOneStepAgentLoopBudgets, counters: ControlledOneStepAgentLoopCounters, reason: ControlledOneStepAgentLoopStopReason, recoverable: boolean, message: string, diagnostics: ControlledOneStepAgentLoopDiagnostic[]): ControlledOneStepAgentLoopState {
  const code: ControlledOneStepAgentLoopDiagnosticCode = reason === "unsafe_metadata" ? "unsafe_metadata" : reason === "budget_exceeded" || reason === "timeout" ? "limit_exceeded" : reason === "malformed_input" ? "malformed_input" : reason === "invalid_transition" ? "invalid_transition" : "policy_blocked";
  return buildState(phase, false, true, budgets, counters, message, [...diagnostics, diagnostic(code, message)], { reason, recoverable, message: safeText(message, 240) });
}

function buildState(phase: ControlledOneStepAgentLoopPhase, enabled: boolean, stopped: boolean, budgets: ControlledOneStepAgentLoopBudgets, counters: ControlledOneStepAgentLoopCounters, summary: string, diagnostics: ControlledOneStepAgentLoopDiagnostic[], stop?: ControlledOneStepAgentLoopStop): ControlledOneStepAgentLoopState {
  return {
    phase,
    authority: "one_step_loop_metadata_only",
    cloudRequired: false,
    ...
    details: buildDetails(budgets, counters),
  };
}

function buildDetails(budgets: ControlledOneStepAgentLoopBudgets, counters: ControlledOneStepAgentLoopCounters): Record<string, string | number | boolean | string[]> {
  const sanitized = sanitizeDisplayValue({ displayOnly: true, sanitized: true, ...budgets, ...counters, repairEnabled: false });
...
function stopMessage(reason: ControlledOneStepAgentLoopStopReason): string {
  ...
  if (reason === "invalid_transition") return "One-step loop event order is invalid for S86.";
