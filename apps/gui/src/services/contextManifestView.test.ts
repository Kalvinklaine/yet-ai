...
const entry = { kind: "file_chunk" as const, sourceRef: "src/main.ts", range: { start: { line: 0, character: 0 }, end: { line: 4, character: 2 } }, symbol: "start", contentHash: hash, inclusionReason: "symbol_match" as const, provenance: "symbol" as const, redaction: "none" as const, byteCount: 100, estimatedTokens: 25, rank: 1 };
...
describe("contextManifestView", () => {
  ...
  it("moves removed entries to omitted and can pin the explicit reference", () => {
    ...
    expect(manifestEntryToExplicitRef(entry)).toEqual({ kind: "file_chunk", sourceRef: "src/main.ts" });
