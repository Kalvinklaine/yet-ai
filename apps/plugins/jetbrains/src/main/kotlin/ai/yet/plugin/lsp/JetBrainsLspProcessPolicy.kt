...
private val secretKeyPattern = Regex("(?i)(?:^|[^A-Za-z0-9])(access[_-]?token|refresh[_-]?token|session[_-]?token|auth[_-]?token|api[-_]?key|apikey|client[_-]?secret|authorization|cookie|password|credential|verifier|secret|token)(?:[^A-Za-z0-9]|$)")
...
private val secretValuePattern = Regex("(?i)\\b[A-Za-z0-9_-]*(?:access[_-]?token|refresh[_-]?token|session[_-]?token|auth[_-]?token|oauth[_-]?code|code[_-]?verifier|pkce[_-]?verifier|verifier|token|secret|api[_-]?key|apikey|client[_-]?secret|authorization|cookie|password|credential)[A-Za-z0-9_-]*\\b\"?\\s*[:=]\\s*\"?[^\\s,;)}\\]]+")
