...
The browser smoke obtains the shipped wrapper HTML through `cd apps/plugins/jetbrains && gradle printSmokeWrapperHtml --quiet --console=plain --args "<panel-origin> <panel-id> <panel-base-path>"`; the Node smoke adds only smoke-only observability hooks around that production output. It registers and loads that wrapper at the production-like `GET /panel/<panel-id>/wrapper.html` route on the same loopback server/origin as `/panel/<panel-id>/index.html`, rather than testing a separate wrapper origin. This is production-like browser coverage, not real IntelliJ/JCEF CI automation.
