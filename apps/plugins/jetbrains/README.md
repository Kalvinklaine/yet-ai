...
This remains a feasibility/lifecycle boundary only. The plugin currently verifies the local runtime through HTTP `/v1/ping`, hosts the GUI in JCEF, sends bounded active editor/selection context through the non-privileged bridge, and keeps JetBrains LSP off unless the developer explicitly opts in to the experimental lifecycle shell. No provider-backed completions, keystroke model calls, edits, indexing, file reads, shell/git/tool execution, or JetBrains apply path are implemented.
