...
This remains a feasibility/lifecycle boundary only. The plugin currently verifies the local runtime through HTTP `/v1/ping`, hosts the GUI in JCEF, sends bounded active editor/selection context through the non-privileged bridge, and keeps JetBrains LSP off unless the developer explicitly opts in to the experimental lifecycle shell. Root `npm run smoke:lsp-stdio` verifies the engine stdio contract directly and is included in `npm run smoke:ide-release-candidate`; JetBrains-specific evidence remains focused Kotlin lifecycle/blocked-decision tests such as `cd apps/plugins/jetbrains && gradle test --console=plain --tests "*JetBrainsLsp*"`. No provider-backed completions, keystroke model calls, edits, indexing, file reads, shell/git/tool execution, JetBrains apply path, native IntelliJ LSP client, or production JetBrains LSP support claim is implemented.
