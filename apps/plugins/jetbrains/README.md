...
The runtime connector supports debug connect mode and local launch mode. In `connect` mode it validates the loopback `runtimeUrl`, reads the optional debug token from PasswordSafe, and checks `GET /v1/ping` with the bearer token when present. In `launch` mode, or `auto` mode when a `yet-lsp` binary is discoverable, it starts the binary with a generated per-session token in `YET_AI_AUTH_TOKEN` and the configured explicit runtime port in `YET_AI_HTTP_PORT`, then checks `/v1/ping`. Launch mode requires a runtime URL with an explicit port such as `http://127.0.0.1:8001`; it does not silently launch on scheme-default ports 80 or 443. Launched processes are stopped when the application service is disposed by calling `destroy()`, waiting briefly, and escalating to `destroyForcibly()` if needed. The connector may discover `yet-lsp` on `PATH`; an explicit `engineBinaryPath` must be absolute and point to an executable file where the platform supports executability checks.
