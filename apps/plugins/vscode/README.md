...
Other GUI messages and IDE action types, including shell, git, task, tool, provider-call, arbitrary file-read/indexing, and edit actions, are not allowlisted and must not call VS Code write/shell/task APIs. Browser fallback paths do not execute these controlled actions in this sprint. JetBrains apply is a separate dev-preview host boundary with deterministic wrapper-browser smoke coverage for the same existing apply/result bridge contract. The confirmed edit-proposal MVP remains the only write path and is local-only with no Yet AI cloud/account/backend requirement; it still requires explicit GUI apply plus host user confirmation and does not allow autonomous edits, model-triggered apply, provider tool execution, shell/tools/tasks/git, file create/delete/rename, apply-patch, arbitrary file reads/indexing, or silent workspace mutation. Tools, tasks, knowledge, shell execution, unconfirmed file edits/apply patch, autonomous indexing, and background autonomy remain disabled in this milestone.
