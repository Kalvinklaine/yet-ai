...
The edit-proposal smoke uses an inline browser harness and a controlled temporary fixture to validate preview-only rendering, explicit user-triggered apply request emission, accepted and denied host confirmation results, failed and rejected host results with repair/retry guidance, the latest-valid-proposal-only apply rule with non-runnable historical bubbles, assistant-supplied request-id rejection, raw-JSON inspect-only/debug copy, stale-result ignore after clear, bounded duplicate completion handling, unsafe path rejection, oversized edit rejection, cleanup, and no secret/private-path leakage. It does not launch VS Code, use providers, call hosted services, run shell/tools/tasks/git, or mutate the real workspace.
...
Include root `npm run smoke:vscode-edit-proposal` in the VS Code edit-proposal gate; that smoke covers preview-only rendering, latest-proposal-only apply, assistant-supplied request-id rejection, raw-JSON inspect-only bubbles, sanitized failed/denied/rejected repair guidance, stale and duplicate result ignore, and the GUI never-edits-files-directly rule without launching real VS Code or calling providers.
