...
Current GUI-to-host receive policy remains deny-by-default except for `gui.ready`, strict controlled IDE actions via `gui.ideActionRequest`, and the confirmed edit-proposal MVP message, `gui.applyWorkspaceEditRequest`. Controlled IDE actions require a bounded unique in-flight request id and only allow `getContextSnapshot`, `openWorkspaceFile`, and `revealWorkspaceRange`. Open/reveal actions resolve existing workspace-relative text files only, reject traversal/absolute/URL-like paths and invalid ranges, and perform navigation only. Context snapshots use bounded active-editor metadata/context and do not read arbitrary files. The host sends sanitized correlated `host.ideActionProgress`/`host.ideActionResult` messages with `cloudRequired: false`.
