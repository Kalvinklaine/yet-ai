...
The client implements a minimal VS Code document/code-intelligence path over stdio JSON-RPC. It reads and bounds server stdout, waits for the `initialize` response, sends `initialized`, synchronizes only supported local `file` text documents with open/change/close notifications, and registers `file`-scheme completion, hover, and document-symbol providers. The deterministic completion proof returns the local status item `Yet AI LSP connected` from cached document state. Hover returns bounded read-only connection status text. Document symbols are bounded names/ranges derived only from cached editor-supplied text. Non-file, untitled, virtual, and remote documents are not synchronized or queried by this MVP.
...
`npm run smoke:vscode-wrapper-browser` is the focused controlled IDE action wrapper smoke for this sprint. It verifies the VS Code-first packaged GUI bridge path for the current read-only/navigation/context actions and does not launch real VS Code, call providers, require a hosted backend, or mutate a workspace. It is automated harness coverage, not manual VS Code dogfood; the T-547 manual VS Code dogfood was not run.
...
The edit-proposal smoke is a local inline browser harness for the confirmed edit proposal preview/apply lifecycle in `apps/plugins/vscode/media/gui`. It drives a controlled temporary fixture and exercises preview-only rendering, explicit user-triggered apply request emission, accepted and denied host confirmation results, failed and rejected host results with sanitized repair/retry guidance, stale-result ignore after clear, bounded duplicate completion handling, unsafe workspace-relative path rejection, oversized edit rejection, browser-storage hygiene, and no secret/private-path leakage. It does not drive real GUI chat bubbles, historical bubbles, or inspect bubbles; the real GUI chat-bubble compact/historical/inspect behavior is covered by `apps/gui/src/App.test.tsx`, and parser, requestId-free envelope, and requestId rejection paths are covered by service/contract tests. It does not launch VS Code, use providers, call hosted services, run shell/tools/tasks/git, or mutate the real workspace.
...
4. Download/read `yet-ai-plugin-manifest-<sha>` (or the per-platform `yet-ai-plugin-manifest-<os>-<arch>-<sha>`) for commit, checksum, and platform metadata.
