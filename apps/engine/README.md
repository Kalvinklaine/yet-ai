...
Engine-owned private JSON stores use one bounded asynchronous atomic-write primitive after each domain has validated its namespace, final-file symlink policy, record shape, and concurrency rules. The primitive creates a unique private temporary file, writes and syncs it, applies `0600` on Unix, atomically replaces or create-links the destination, removes the temporary file on success or failure, and optionally syncs the parent directory. Parent sync is best-effort only for platform/filesystem errors that do not support directory sync; non-Unix platforms rely on their private user-storage boundary, and Windows replacement uses replace-existing write-through semantics. Provider configs, provider test state, chat history, project memory, demo mode, and protected secret files request parent sync. The projects registry retains its separate lock/revision transaction, and callers retain all root confinement, symlink, schema, and put-if-absent policy.
