...
- store access tokens, refresh tokens, API keys, and revocation state through the engine-owned composite secret store, using OS credential storage where available and protected-file fallback when the keychain backend is unavailable;
...
Production builds use the composite secret-store policy: OS credential storage is the preferred primary backend where the configured platform keychain service is available, and the protected-file backend under user config remains the fallback for unavailable, locked, unsupported, headless, dev, or CI cases. Debug/test builds deliberately use an unavailable primary plus the protected-file fallback so automated tests do not prompt or depend on a real keychain. Existing fallback records can migrate to keychain only after a verified write/read-back; keychain values win over stale fallback records; fallback records never overwrite newer keychain secrets; and delete/disconnect paths attempt to delete both keychain and fallback records where applicable. This is local custody for BYOK credentials, not hosted custody, cloud sync, enterprise secret management, production compliance certification, official production OAuth support, or a guarantee that every platform has an unlocked credential service. Raw provider secrets, access or refresh tokens, authorization codes, PKCE verifiers, cookies, browser profiles, credential file paths, and other provider secret material must never be GUI-owned, stored by GUI/browser code, returned through GUI-facing responses, or used in fixtures.
...
Fallback-to-keychain migration is also conservative. When a production keychain lookup is empty and a fallback record exists, the composite store writes the fallback value to keychain and deletes the fallback record only after reading back the same value. If the read-back fails or differs, the fallback record is preserved and a sanitized storage error is returned. When keychain already has a value, that value wins over fallback and the fallback never overwrites it. Delete and provider-auth disconnect paths call the same secret-store delete operation for API-key and OAuth secret kinds, so they attempt to remove both keychain and fallback records where applicable. These behaviors are local credential custody only, not cloud sync, not hosted custody, not enterprise secret management, not official production OpenAI/ChatGPT OAuth support, and not a guarantee that every platform has an unlocked credential service.
...
- OS credential storage is preferred only where the platform service is available and usable. The protected-file backend remains the documented fallback for headless, dev, CI, locked-keychain, or unavailable-keychain cases; this is not cloud sync, hosted custody, enterprise secret management, or a production compliance guarantee.
