...
The planned LSP MVP is read-only and not part of the current HTTP API. Its approved scope is limited to local `initialize`, `initialized`, `shutdown`, `exit`, bounded `textDocument/didOpen`, `textDocument/didChange`, `textDocument/didClose`, and deterministic `textDocument/completion` or status proof over in-memory documents supplied by the editor. It must not read arbitrary files, index workspaces, call providers or local models on keystrokes, execute shell/tools, write files, apply patches, mutate workspaces, expose raw document bodies in logs, or require a hosted Yet AI backend, account, managed gateway, product credits, or cloud workspace.
