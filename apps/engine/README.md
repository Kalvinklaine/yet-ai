...
`../../docs/architecture/042-project-context-platform.md` and `../../docs/architecture/043-project-context-threat-model.md` define the engine-owned project-context boundary. Implemented responsibilities include one rebuildable SQLite context cache per project, safe deterministic inventory, a generation-bound deterministic project profile, internal FTS5 lexical-first retrieval over bounded admitted source chunks, bounded tree-sitter definition metadata for Rust and TypeScript/JavaScript ranking, and authenticated generation-bound planning with an inspectable `ContextManifest`. Provider dispatch integration remains planned. Immutable effective manifests, turn links, generation lineage, continuation hashes, and partial assistant text are separate planned durable per-project config records. Cache deletion must never remove that turn evidence or the separate implemented project-memory store of explicit user-authored notes.

Each immutable project context resolves one rebuildable SQLite cache under project cache storage plus a separate reserved durable turn-context path under project config storage. Authenticated status, explicit rebuild, profile, and `POST /p/{project_id}/v1/context/plan` routes are live. Rebuild respects repository ignore files and product exclusions, does not follow symlinks, and atomically commits relative inventory/profile facts together with deterministic bounded admitted source chunks, lightweight definition metadata, and current-generation FTS5 rows. Planning accepts only bounded project-relative explicit refs and cached generation-scoped evidence, ranks explicit selections before lexical/symbol/profile candidates, reports policy and budget omissions, and persists its rebuildable plan record in the context cache. It performs no provider request and grants no tool authority. Symbol metadata carries content hash, parser source, confidence, and validated project-relative ranges with one-based lines and zero-based UTF-8 byte columns. Supported syntax uses tree-sitter; malformed supported syntax may use a bounded lower-confidence heuristic fallback. Neither source provides call/reference resolution or compiler-accurate graph semantics. The cache is local, unencrypted, isolated per project, and permission-restricted where supported. Public lexical/symbol query and cache-delete routes are not exposed. No cache text is automatically attached or uploaded. Durable turn persistence, partial-response persistence, Continue, and watchers remain unsupported. GUI-facing contracts use opaque project IDs and project-relative source refs; canonical roots remain private.
...
Chats remain provider-neutral local history. Creation stores neither provider credentials nor a bound model; every accepted send resolves current engine-owned provider, local runtime, Demo Mode, and experimental account-auth state in precedence order. A chat created before login therefore keeps its id and history after successful login and can use the newly connected state on its next send. API-key/OpenAI-compatible and local-provider readiness, plus Demo Mode where enabled, continue to win over the experimental account fallback.

On the experimental Codex-like path only, a pre-stream model rejection can cause one bounded model-catalog rediscovery. The engine excludes the exact rejected candidate, chooses the first remaining eligible model in returned order, persists it in engine-owned auth metadata, and retries that send once. Non-model invalid requests do not rediscover. If discovery fails, returns no alternate, or yields no changed eligible model, the request fails closed with a sanitized model-invalid category. This is no guarantee that every model is available or usable. Raw catalogs, provider bodies, account metadata, auth/session values, and provider headers are never returned through GUI-facing status or SSE errors.
...
`GET /v1/agent-progress` is a local observability endpoint for the current agent progress MVP. It returns the strict list-response shape with `cloudRequired: false` and `providerAccess: "direct"`. The endpoint reads the engine-owned local cache source `agent-progress/progress.json` when it exists, returns populated sanitized snapshots from that source, overlays any in-memory metadata submitted through `POST /v1/agent-progress/events`, and returns an empty `snapshots` array when no source or in-memory events exist. In normal runtime storage this source is the Yet AI cache file resolved by the writer helper as `<cacheRoot>/yet-ai/agent-progress/progress.json`; tests and explicit local workflows may override the state file with `YET_AI_AGENT_PROGRESS_STATE` or an explicit `--state` path. The public `progress.json` file is an `AgentProgressListResponse` that the endpoint consumes directly. Internal event accumulation is stored separately by the JS helper in a sibling internal events file and should not be consumed by the engine or GUI.

`POST /v1/agent-progress/events` is an authenticated loopback metadata-only publishing path for explicit developer workflows and bounded GUI/IDE host activity. It accepts only bounded sanitized progress fields aligned to the engine agent-progress event contract: ids, phase/status, safe summary/message, safe tool kind/label, heartbeat timestamps, and bounded sanitized output tails. The currently wired product producer is project-scoped VS Code controlled read/search/edit/multi-file/command/verification activity after explicit user requests and matching `live_host` provenance; scripts, smokes, and direct developer calls are separate local-derived producers. Normal chat does not publish progress, Browser and JetBrains controlled execution do not produce those host-action events, and no producer supplies a complete autonomous lifecycle. The engine observes metadata but does not execute IDE tools, shell/git/tasks, open files, apply edits, read arbitrary files, call providers because of an event, or mutate the workspace.

The controlled IDE action host path is limited to local bounded read-only/navigation/context actions in VS Code and JetBrains: `getContextSnapshot`, `openWorkspaceFile`, and `revealWorkspaceRange`. Browser fallback does not execute controlled actions. The engine's role is metadata-only observability through the sanitized progress/event endpoints; it is not an IDE tool executor and does not gain shell/git/task/tool authority, arbitrary file-read/indexing authority, provider-call authority for IDE actions, or workspace mutation authority.

Future tool authority and edit/apply expansion is tracked in `../../docs/architecture/010-tool-authority-and-edit-pipeline-roadmap.md`. The engine boundary remains deny-by-default and contract-first: no generic tools, patch execution, shell/git/task execution, hidden reads/search/indexing, provider tool execution, model-triggered verification, create/delete/rename/move authority, or automatic proposal application is implemented by this roadmap.

Corrupt, invalid, oversized, or unsafe source/event data is reported only as sanitized unavailable/error text and must not echo raw file data, private paths, auth-file names, parser dumps, storage internals, submitted commands, or raw bodies. Agent-progress surfaces must not start, stop, merge, apply, execute tools, run shell commands, call providers, read task-board state, mutate workspaces, or require hosted services or cloud sync. Non-empty snapshots may include only safe operational fields such as ids, phase/status, tool label/kind, elapsed and heartbeat ages, stuck reason, recent summaries, and bounded sanitized output tails. They must not include prompts, chain-of-thought, raw file contents, raw provider responses, tokens, cookies, provider credentials, runtime session tokens, credential paths, private absolute paths, shell scripts, execution commands, git output, task output, or patch payloads.
...
Engine-owned private JSON stores use one bounded asynchronous atomic-write primitive after each domain has validated its namespace, final-file symlink policy, record shape, and concurrency rules. The primitive creates a unique private temporary file, writes and syncs it, applies `0600` on Unix, rechecks final-file symlinks immediately before commit, atomically replaces or create-links the destination, and removes the temporary file on success or failure. Parent-directory durability is explicit per caller: project memory uses strict sync; chat history, demo mode, protected secrets, and provider-auth pending/session-registry state ignore only unsupported directory-sync errors; provider config and provider test state preserve their no-parent-sync policy. Non-Unix platforms rely on their private user-storage boundary, and Windows replacement uses replace-existing write-through semantics. The projects registry retains its separate lock/revision transaction, and callers retain all root confinement, schema, and put-if-absent policy.
...
Experimental account login is explicit-risk and non-default. For the approved Codex-like OpenAI path, `start` returns an authorization URL whose redirect URI is the engine-owned loopback listener at `http://localhost:1455/auth/callback`; the listener captures the browser callback locally, exchanges the code through engine-owned storage, and returns only short sanitized browser text. Manual authorization-code exchange remains a fallback for cases where the browser callback cannot complete. The API-key fallback remains the safe/default real-provider route, and no hosted Yet AI backend, Yet AI account, cloud workspace, managed gateway, or product credits are required.

Successful exchange changes shared engine-owned auth/model readiness, not chat identity. GUI callback polling or manual exchange may refresh shared client state, but the authoritative provider and model are still selected by the engine on each send. No chat recreation or credential/model migration into chat history is required.

The focused documentation-only convergence gate is `npm run validate:docs`, `npm run check:agent-architecture-contract`, `npm run validate:hygiene`, and `git diff --check` from the repository root. These static checks do not rerun engine tests or loopback smokes and are not real-account, all-model, official-support, or production evidence.
...
The current provider-auth framework is adapter-oriented even though the public route envelope remains the existing `/v1/provider-auth/{provider}/...` compatibility API. `provider_auth/mod.rs` coordinates provider validation, request validation, API-key fallback checks, mock-test compatibility, and hardened local state helpers; provider-specific login work is dispatched through `provider_auth/adapters/mod.rs`. The current production-visible adapter is the explicit-risk `openai` Codex-like adapter. The adapter SPI declares provider id, display label, auth modes, policy gates, sanitized status projection, browser callback hooks, refresh, disconnect, and chat-auth snapshot support. A shared session registry stores pending session id/state/mode/expiry/callback-owner metadata for callback validation and retry/terminal cleanup. The test-only device-flow proof uses the same adapter dispatch and registry shape without enabling production `openai-compatible` OAuth login, a public device-flow mode selector, or public `userCode`/complete-verification-URI UX.

Current implemented provider-auth surface is narrower than the target framework: API-key fallback, mock OAuth test harness, experimental `openai` Codex-like browser PKCE/manual/callback path, adapter dispatch/session registry plumbing, and test-only device-flow proof coverage. Target device-flow fields such as user code and complete verification URI remain future contract work until separately implemented and versioned.

- start the experimental browser/manual/callback login path and hold pending PKCE/session state locally;
...
- receive loopback callback or exchange requests for the implemented experimental path; polling remains limited to mock/test or future device-flow work;
...
- refresh or disconnect credentials where implemented without exposing raw secrets;
- return the narrow public status projection described below; provider/exchange/storage detail remains internal and must not be reconstructed from provider bodies.

The canonical stored-auth classifier is metadata/access-token first:

- an access-only bundle is chat-usable until its recorded expiry, then requires reconnect or API-key fallback;
- an unexpired refreshable bundle is usable and a near-expiry or expired refreshable bundle enters refresh;
- a transient refresh failure, including any 5xx response, does not quarantine or clear credentials and retains the stored OAuth bundle so a later attempt can recover;
- only exact, case-sensitive permanent codes in top-level `error` or direct nested `error.code` quarantine that refresh token: `refresh_token_reused`, `invalid_grant`, `refresh_token_revoked`, and `revoked`;
- permanent quarantine is process-local and keyed by a hash of config directory, provider, and refresh token. A quarantine hit avoids another provider call, clears only the OAuth bundle, and preserves API-key configuration;
- missing, incomplete, invalid, or expired-without-refresh auth does not become chat auth.

The public provider-auth projection uses statuses `login_unavailable`, `api_key_configured`, `login_available`, `pending`, `connected`, `expired`, `revoked`, and `error`; the GUI client also has a local `not_configured` state. Public `authSource` values are only `none`, `api_key`, and `oauth`. Internal `ProviderError`, `ExchangeFailed`, and `StorageError` states collapse to public `error`. GUI recovery copy stays generic and must not expose internal category codes, provider descriptions, account identifiers, scopes, token hints, raw callback values, or storage paths.
...
`npm run smoke:local` is a local-only cross-subsystem smoke test. It starts the engine on a free loopback port through Cargo, starts mock OpenAI-compatible, experimental token, and experimental chat endpoints, configures a fake provider key, sends chat commands, reads SSE streams, checks provider Authorization internally, verifies local chat history persistence/snapshot hydration/delete behavior, exercises provider-auth default status plus the local mock OAuth start/exchange/status/disconnect flow, and covers the approved experimental Codex-like start, engine-owned `localhost:1455/auth/callback` capture, manual exchange fallback, and chat fallback through loopback mocks only. Callback coverage proves successful exchange returns HTTP 200 and connected status, a transient mock HTTP 502 returns sanitized HTTP 502 browser/status diagnostics while retaining the pending session for retry, and only canonical HTTP 400 plus top-level `invalid_grant` removes retry state and requires a fresh login. It asserts fake API keys, OAuth access tokens, refresh tokens, Authorization headers, cookies, PKCE verifier values, mock auth codes, provider descriptions/bodies, Codex credential-file paths, raw callback query markers, callback state/code values, private paths, and local history responses/events do not leak through client-visible output or engine logs. It requires Cargo on `PATH` and does not require real provider credentials, external network access, or hosted Yet AI services. Real experimental OpenAI/ChatGPT account testing remains manual, high-risk, and outside CI; the API-key fallback remains the safe/default real-provider path.
...
6. Before callback tests, inspect port `1455`. Stop only a process confirmed to be an orphaned test process; never stop a production runtime without user approval.
7. For login failures, first verify API-key fallback status, then confirm experimental start was explicitly requested, then check fixed loopback callback ownership on `localhost:1455`, persisted pending-session registry state, token endpoint result category, and secret-store cleanup/refresh behavior. The persisted registry is callback authority: the in-memory state map only caches known config directories and candidate mappings, every callback is revalidated against persisted state, and missing or ambiguous matches evict the cache and fail closed.
8. Treat browser success as evidence only after authorization-code exchange and credential storage both succeeded. Callback receipt by itself is never success.
9. Internal diagnostics may use `token_http_failed_or_timeout`, a real `token_http_status_<status>`, `provider_rejected`, `refresh_token_reused`, `adapter_failure`, token-shape categories, and `storage_failed`. Never invent HTTP status zero for a non-HTTP failure, and never log or display raw provider response bodies to explain a category.
10. Keep reports sanitized: no raw callback URL with code/state, authorization header, token, cookie, PKCE verifier, provider body, account id, scope list, token hint, or private path.
11. Ignore callback-page DevTools Content Security Policy warnings for blocked image or connection probes. The callback page intentionally uses `default-src 'none'`; those warnings do not cause token exchange failure.
12. A callback HTTP 502 is causal evidence that the loopback listener received the callback but token exchange, token validation, model discovery, or credential storage failed afterward. Check the sanitized provider-auth `lastError` taxonomy and the installed runtime log instead of treating it as a listener/CSP failure.
13. In an installed JetBrains build, open Tools → `Yet AI: Show Runtime Status`, then Tools → `Yet AI: Open Logs Folder`. Inspect `engine-<port>.log` for the allowlisted `provider_auth.exchange_failed` event and compare `Build commit` plus `Runtime binary freshness` with the artifact/check-out being tested. Rebuild and reinstall before network diagnosis when the commit differs or a normal bundled `auto`/`launch` runtime is not `bundled match`.
14. For transient transport or 5xx categories, retry the retained pending session once or use manual authorization-code exchange. For canonical HTTP 400 `invalid_grant`, start a fresh login and never reuse the code. The API-key fallback remains available in both cases.
...
- Controlled IDE action execution is not an engine feature. Current controlled actions are host-mediated read-only/navigation/context actions only in VS Code and JetBrains; browser fallback does not execute them.
- No full agent autonomy, indexing, tool registry execution, controlled IDE action execution by the engine, file mutation, shell execution, git/task execution, integrations, production LSP completion/code-lens, provider-backed completions, tools, tasks, knowledge, or file edits are complete. The current LSP surface is limited to the implemented read-only stdio document lifecycle plus deterministic local completion and hover status proofs and bounded document-symbol support, with opt-in VS Code wiring only.
...
- Provider/caps v2 is planned only. The current engine emits MVP model metadata with chat, streaming, tools, and reasoning booleans plus `ready`, `disabled`, `missing_credentials`, `missing_model`, and `unsupported` readiness. Future fields such as capability provenance, optional context-window classes, local-provider availability facts, tested-vs-configured readiness, or provider-family labels must be sourced and sanitized by the local runtime before they become contracts. Those fields would be display/routing hints only and must not grant shell, git, tool, edit, indexing, provider-tool, autonomous-agent, or workspace-mutation authority. Do not add built-in provider catalogs, model snapshots, dynamic provider claims, or exact context-window claims without implemented local validation.
