...
The controlled IDE action host path is VS Code-first for this sprint and is limited to local bounded read-only/navigation/context actions: get context snapshot metadata, open an existing workspace-relative file, and reveal a bounded workspace-relative range. Browser and JetBrains do not execute controlled actions in this sprint. The engine's role is metadata-only observability through the sanitized progress/event endpoints; it is not an IDE tool executor and does not gain shell/git/task/tool authority, arbitrary file-read/indexing authority, provider-call authority for IDE actions, or workspace mutation authority.
...
- Controlled IDE action execution is not an engine feature. Current controlled actions are VS Code-first host-mediated read-only/navigation/context actions only; browser and JetBrains do not execute them in this sprint.
