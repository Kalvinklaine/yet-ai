...
`POST /v1/agent-progress/events` is an authenticated loopback metadata-only publishing path for GUI/IDE hosts to expose controlled IDE action progress awareness. It accepts only bounded sanitized progress fields aligned to the engine agent-progress event contract: ids, phase/status, safe summary/message, safe tool kind/label, heartbeat timestamps, and bounded sanitized output tails. Controlled IDE actions remain bridge/host-mediated in this sprint: the engine may observe this metadata but does not execute IDE tools, shell/git/tasks, open files, apply edits, read arbitrary files, call providers because of the event, or mutate the workspace. Confirmed edit proposals remain the only intended write path outside this metadata surface.
