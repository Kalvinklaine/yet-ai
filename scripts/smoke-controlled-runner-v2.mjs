...
const modelId = "controlled-runner-v2-smoke-model";
const providerId = "controlled-runner-v2-smoke-provider";
const memoryNote = {
  id: "controlled-runner-v2-memory-note-1",
  title: "Controlled runner memory",
  text: "Prefer explicit user review before applying controlled runner v2 task edits.",
  tags: ["controlled", "smoke"],
  source: "manual",
  createdAt: "2026-06-18T12:00:00Z",
  updatedAt: "2026-06-18T12:00:00Z",
};
const activeFileText = "export function controlledRunnerAnswer() {\n  return 'old value';\n}\n";
const snippetText = "function controlledRunnerSnippet() { return 'context'; }";
const taskGoal = "Update the controlled runner copy after reviewing explicit local context.";
const userPrompt = "Goal: Update the controlled runner copy after reviewing explicit local context.";
const verificationTail = "Repository check passed for controlled runner v2 task smoke.";
const rawMarkers = [
  "Bearer controlled-runner-v2-raw-secret",
  "sk-controlled-runner-v2-secret-00000000",
  "authorization: bearer",
  "cookie: controlled-runner-v2",
  "raw prompt",
  "provider response raw dump",
  "/Users/Controlled/private/project",
];
const failures = [];
...
try {
  ...
  await expectVisibleText(page, "Coding task session", "controlled runner v2 task panel", 20_000);
  await expectVisibleText(page, "Ready to send using controlled-runner-v2-smoke-model through the local runtime.", "mock model readiness", 20_000);
  ...
  await expectTextareaValue(page, taskGoal, "local controlled task prompt draft");
  assert(commandCount === 0, "drafting controlled task prompt auto-sent chat");
  ...
  await dispatchHostMessage(page, {
    ...
    payload: {
      status: "succeeded",
      message: "Returned one controlled active-file excerpt.",
      cloudRequired: false,
      action: "getActiveFileExcerpt",
      contextAttachment: {
        kind: "active_file_excerpt",
        source: "vscode",
        file: { displayPath: "src/controlled-runner.ts", workspaceRelativePath: "src/controlled-runner.ts", languageId: "typescript" },
  ...
  await expectVisibleText(page, "Result excerpt: src/controlled-runner.ts", "mock active editor excerpt", 20_000);
  await page.getByRole("button", { name: "Add to multi-file context bundle" }).click();
  await expectVisibleText(page, "active file excerpt · src/controlled-runner.ts", "active excerpt in controlled session summary", 20_000);

  await page.getByLabel("Literal snippet query").fill("controlledRunnerSnippet");
  ...
  await dispatchHostMessage(page, {
    ...
    payload: {
      status: "succeeded",
      message: "Returned one controlled runner v2 snippet.",
      cloudRequired: false,
      action: "searchWorkspaceSnippets",
      queryLabel: "controlledRunnerSnippet",
      resultCount: 1,
      truncated: false,
      snippets: [{ workspaceRelativePath: "src/controlled-snippet.ts", languageId: "typescript", range: { start: { line: 1, character: 0 }, end: { line: 1, character: 43 } }, text: snippetText }],
    },
  });
  await expectVisibleText(page, "1 sanitized snippet", "snippet search result", 20_000);
  await page.locator("label", { hasText: "src/controlled-snippet.ts" }).locator("input[type='checkbox']").check();
  await page.getByRole("button", { name: "Attach selected snippets (1)" }).click();
  await expectVisibleText(page, "project snippet · src/controlled-snippet.ts", "snippet in controlled session summary", 20_000);
  ...
  await expectVisibleText(page, "project memory · Controlled runner memory", "memory in controlled session summary", 20_000);
  await expectVisibleText(page, "Memory attachments: 1", "controlled memory count", 20_000);
  ...
  await expectVisibleText(page, "Edit proposed/applied: proposal visible for review", "controlled safe proposal status", 20_000);
  ...
  await dispatchHostMessage(page, {
    ...
    payload: {
      ...
      cloudRequired: false,
      appliedEditCount: 1,
      affectedFiles: ["src/controlled-runner.ts"],
    },
  });
  ...
  await expectVisibleText(page, "Verification/follow-up: repository-check: succeeded", "controlled verification status", 20_000);
  await page.getByRole("button", { name: "Draft verification follow-up prompt" }).click();
  await expectTextareaValue(page, "Verification follow-up prompt", "manual controlled runner follow-up draft");
  await expectTextareaValue(page, verificationTail, "manual controlled runner verification output draft");
  assert(commandCount === 1, "drafting controlled runner follow-up prompt auto-sent chat");
  await page.getByRole("button", { name: "Attach verification result to next message" }).click();
  await expectVisibleText(page, "Verification/follow-up: repository-check: succeeded · attached for follow-up", "controlled verification follow-up cue", 20_000);
  await expectVisibleText(page, "verification output · repository-check · succeeded", "verification output bundle summary", 20_000);
  ...
  assert(!storageText.includes(memoryNote.text) && !storageText.includes(taskGoal), "controlled task memory or goal persisted in browser storage");
  assertNoRawMarkers(JSON.stringify(pageState), "DOM or browser storage");

  if (failures.length > 0) {
    throw new Error(`Controlled runner v2 task smoke failed:\n${failures.map((failure) => `- ${failure}`).join("\n")}`);
  }

  console.log("Controlled runner v2 task smoke passed.");
  console.log("Verified deterministic mock-only controlled runner v2 manual lifecycle: local goal, explicit context/memory/snippet attachment, one explicit mock-provider send, safe edit proposal, explicit apply result, explicit verification result attachment, no auto-send/apply/run, no browser-storage persistence, no non-loopback network, and no shell/git/tool execution.");
} catch (error) {
...
async function buildGui() {
  ...
  if (result.status !== 0) {
    failActionable("GUI build failed.", ["Run `cd apps/gui && npm install` if dependencies are missing, then retry `npm run smoke:controlled-runner-v2`."]);
  }
}
...
async function mockRuntimeResponse(value, method, body) {
  ...
  if (method === "POST" && url.pathname === "/v1/project-memory/search") {
    return json({ queryLabel: "controlled", matches: [{ note: memoryNote, scoreLabel: "exact" }], cloudRequired: false, providerAccess: "direct" });
  }
...
function providerSummary() {
  return {
    ...
    displayName: "Controlled Runner v2 Smoke Provider",
    enabled: true,
...
function chatThread(messages) {
  return { chatId, title: "Controlled runner v2 smoke", createdAt: "2026-06-18T12:00:00Z", updatedAt: "2026-06-18T12:00:00Z", messages };
}

function assistantMessage() {
  return {
    id: "controlled-assistant-proposal-1",
    chatId,
    role: "assistant",
    status: "complete",
    ...
    content: JSON.stringify({
      ...
      payload: {
        ...
        cloudRequired: false,
        summary: "Replace controlled runner answer after explicit review.",
        edits: [{
          workspaceRelativePath: "src/controlled-runner.ts",
          textReplacements: [{
...
function failActionable(summary, lines) {
  console.error(`Controlled runner v2 task smoke failed: ${summary}`);
