...
const modelId = "real-coding-dogfood-model";
const providerId = "real-coding-dogfood-provider";
const memoryNote = {
  id: "dogfood-memory-note-1",
  title: "Real coding dogfood memory",
  text: "Prefer explicit user review before applying local coding task edits.",
  tags: ["dogfood", "smoke"],
  source: "manual",
  createdAt: "2026-06-18T12:00:00Z",
  updatedAt: "2026-06-18T12:00:00Z",
};
const activeFileText = "export function dogfoodAnswer() {\n  return 'old value';\n}\n";
const snippetText = "function dogfoodSnippet() { return 'context'; }";
const taskGoal = "Update the dogfood answer copy after reviewing explicit local context.";
const userPrompt = "Goal: Update the dogfood answer copy after reviewing explicit local context.";
const assistantAnswer = "Mock coding answer: reviewed the explicit memory, active file excerpt, and snippet. Change dogfoodAnswer to return 'new value' after human review.";
const rawMarkers = [
  "Bearer dogfood-coding-raw-secret",
  "sk-dogfood-coding-secret-00000000",
  "authorization: bearer",
  "cookie: dogfood-coding",
  "raw prompt",
  "provider response raw dump",
  "/Users/Dogfood/private/project",
];
const failures = [];
...
try {
  ...
  await expectTextareaValue(page, taskGoal, "local coding task prompt draft");
  assert(commandCount === 0, "drafting task prompt auto-sent chat");
  ...
  await dispatchHostMessage(page, {
    ...
    payload: {
      status: "succeeded",
      message: "Returned one dogfood active-file excerpt.",
      cloudRequired: false,
      action: "getActiveFileExcerpt",
      contextAttachment: {
        kind: "active_file_excerpt",
        source: "vscode",
        file: { displayPath: "src/dogfood.ts", workspaceRelativePath: "src/dogfood.ts", languageId: "typescript" },
  ...
  await expectVisibleText(page, "Result excerpt: src/dogfood.ts", "mock active editor excerpt", 20_000);
  await page.getByRole("button", { name: "Add to multi-file context bundle" }).click();
  await expectVisibleText(page, "active file excerpt · src/dogfood.ts", "active excerpt in coding task summary", 20_000);
  await assertTraceEntries(page, ["IDE action requested", "IDE action result received"], "active excerpt trace");

  await page.getByLabel("Literal snippet query").fill("dogfoodSnippet");
  ...
  await dispatchHostMessage(page, {
    ...
    payload: {
      status: "succeeded",
      message: "Returned one dogfood coding snippet.",
      cloudRequired: false,
      action: "searchWorkspaceSnippets",
      queryLabel: "dogfoodSnippet",
      resultCount: 1,
      truncated: false,
  ...
  await expectVisibleText(page, "project snippet · src/snippet.ts", "snippet in coding task summary", 20_000);
  ...
  await expectVisibleText(page, assistantAnswer, "mock coding answer", 20_000);
  await assertTraceEntries(page, ["Send requested", "Send accepted", "Chat stream finished"], "send and mock response trace");
  ...
  await assertTraceSanitizedAndBounded(page, [activeFileText, snippetText, memoryNote.text, taskGoal, userPrompt]);
  ...
  assert(!storageText.includes(memoryNote.text) && !storageText.includes(taskGoal) && !storageText.includes(activeFileText) && !storageText.includes(snippetText), "memory, context, snippet, or goal persisted in browser storage");
  assertNoRawMarkers(JSON.stringify(pageState), "DOM or browser storage");

  if (failures.length > 0) {
    throw new Error(`Real coding task dogfood smoke failed:\n${failures.map((failure) => `- ${failure}`).join("\n")}`);
  }

  console.log("Real coding task dogfood smoke passed.");
  console.log("Verified deterministic mock-only real coding task dogfood loop: real-provider-like loopback readiness, local goal draft, explicit memory/snippet/context attachment, one manual mock-provider send, one explicit context bundle, no auto-send/apply/run, no browser-storage persistence, no non-loopback network, and no shell/git/tool execution.");
} catch (error) {
...
async function buildGui() {
  ...
  if (result.status !== 0) {
    failActionable("GUI build failed.", ["Run `cd apps/gui && npm install` if dependencies are missing, then retry `npm run smoke:real-coding-task-dogfood`."]);
  }
}
...
async function mockRuntimeResponse(value, method, body) {
  ...
  if (method === "POST" && url.pathname === "/v1/project-memory/search") {
    return json({ queryLabel: "dogfood", matches: [{ note: memoryNote, scoreLabel: "exact" }], cloudRequired: false, providerAccess: "direct" });
  }
...
function providerSummary() {
  return {
    ...
    displayName: "Real Coding Dogfood Mock Provider",
    enabled: true,
...
function chatThread(messages) {
  return { chatId, title: "Real coding dogfood smoke", createdAt: "2026-06-18T12:00:00Z", updatedAt: "2026-06-18T12:00:00Z", messages };
}

function assistantMessage() {
  return {
    id: "dogfood-assistant-answer-1",
    chatId,
    role: "assistant",
    status: "complete",
...
async function assertTraceSanitizedAndBounded(page, forbiddenValues) {
  ...
  for (const value of forbiddenValues) {
    assert(!value || !traceText.includes(value), "raw prompt, context, or memory body leaked into coding session trace");
  }
...
async function assertNoForbiddenBridgeActions(page) {
  ...
  const allowed = new Set(["getActiveFileExcerpt", "searchWorkspaceSnippets"]);
  ...
  assert(messages.filter((message) => message?.type === "gui.applyWorkspaceEditRequest").length === 0, "unexpected apply request in dogfood answer smoke");
...
function failActionable(summary, lines) {
  console.error(`Real coding task dogfood smoke failed: ${summary}`);
