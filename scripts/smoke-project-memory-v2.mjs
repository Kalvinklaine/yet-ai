...
const token = `smoke-project-memory-v2-token-${crypto.randomUUID()}`;
const fakeApiKey = `sk-smoke-project-memory-v2-${crypto.randomUUID()}`;
const providerId = `smoke-project-memory-v2-${Date.now()}`;
const modelId = "smoke-project-memory-v2-model";
...
const assistantText = "Project memory v2 smoke acknowledgement.";
...
try {
  ...
  await smoke.sendHostReady({
    ...
    sessionToken: token,
    workspaceBinding: { state: "auto_bound", projectId: "prj_abcdefghijklmnopqrstuA", displayName: "Project memory v2 smoke workspace" },
  });
  ...
  assert(countOccurrences(providerPrompt, "IDE context bundle") === 1, "provider prompt included explicit context bundle more than once");
  assert(countOccurrences(providerPrompt, `project memory noteId=`) === 1, "provider prompt included project memory metadata more than once");
  assert(countOccurrences(providerPrompt, `title=${noteTitle}`) === 1, "provider prompt included memory title more than once");
  assert(countOccurrences(providerPrompt, `tags=${noteTag}`) === 1, "provider prompt included memory tag more than once");
  assert(countOccurrences(providerPrompt, noteText) === 1, "provider prompt included memory note text more than once");
  assert(countOccurrences(providerPrompt, userMessage) === 1, "provider prompt included user message more than once");
  ...
  if (failures.length > 0) {
    throw new Error(`Project memory v2 smoke failed:\n${failures.map((failure) => `- ${failure}`).join("\n")}`);
  }

  console.log("Project memory v2 smoke passed.");
...
async function makeTempHome() {
  const home = path.join(os.tmpdir(), `yet-ai-project-memory-v2-smoke-${process.pid}-${Date.now()}`);
