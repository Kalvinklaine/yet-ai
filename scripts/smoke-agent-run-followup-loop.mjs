...
async function runFailedVerificationFixDraftScenario(guiBaseUrl) {
  ...
  await expectVisibleText(page, "Drafting only writes the composer and focuses it. It never sends, applies, runs verification, attaches context, saves memory, changes readiness, or stores browser data.", "draft-only boundary copy", 20_000);
  ...
  assert.equal(draft.includes("Repository fixture check failed."), true, "fix draft omitted sanitized output tail");
  assert.equal(draft.includes("Suggest the smallest safe fix plan"), true, "fix draft omitted fix intent");
