...
`AgentProgressListResponse` is the future read-only engine/GUI list surface for local progress observability. It returns `cloudRequired: false`, `providerAccess: "direct"`, an optional bounded UTC `generatedAt`, and a bounded `snapshots` array aligned with the strict sanitized snapshot shape. Empty lists are valid. The response does not define start, stop, merge, apply, write, shell, tool, provider, hosted-backend, account, or cloud-workspace actions.
