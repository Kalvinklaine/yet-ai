...
The `searchWorkspaceSnippets` controlled action is a contract-only preview for explicit user-initiated local workspace snippet search without indexing. The GUI request payload is exactly `{ "action": "searchWorkspaceSnippets", "query": "literal bounded words" }` with a non-empty sanitized literal query; it rejects regex/glob/path syntax, cwd/env/shell/git/tool/provider/model/API-key fields, assistant-supplied request correlation, empty or overlong values, and unknown fields. A successful host result returns only sanitized `queryLabel`, bounded `resultCount`, bounded `snippets` with `workspaceRelativePath`, `languageId`, ordered `range`, bounded sanitized `text`, plus `truncated` and `cloudRequired: false`. Rejected, unavailable, or failed results must not include snippet text. This contract is local-first prompt-context discovery only: no background indexing, arbitrary path reads, full-file access, shell/git/tasks/tools/provider calls, model-triggered search, workspace mutation, hosted backend requirement, or cloud workspace is granted.
...
A verification result may become prompt context only through a separate explicit user click after the host result is available. Chat commands represent that attachment as an `explicit_context_bundle` item with `kind: "verification_output"`, allowlisted `commandId`, bounded `exitCode`, `status` of `succeeded` or `failed`, bounded sanitized `outputTail`, and `truncated`. The item rejects raw command strings, cwd, environment, shell scripts, provider/model/API-key fields, private paths, secrets, unknown fields, and unbounded output. This keeps the edit→verify follow-up loop user-driven and one-shot; assistants cannot request verification runs or smuggle executable command material back through prompt context.
