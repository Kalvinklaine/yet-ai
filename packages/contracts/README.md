...
- Current bridge schemas are strict only for `gui.ready`, `host.ready`, `host.openedFromCommand`, and non-privileged `host.contextSnapshot`; privileged GUI/plugin messages remain disabled until strict schemas, policy, request correlation, and confirmation are implemented.
- Current chat command schemas are strict only for non-privileged `user_message` with optional bounded active editor context and `abort`; tool decisions, IDE tool results, parameter changes, message mutation, regeneration, file edits, shell-like actions, and workspace mutation remain disabled until schema, policy, and confirmation are implemented.
