...
| `project_context` | `live_engine` | `apps/engine/src/project_context/`, `apps/engine/src/http/project.rs`, `apps/engine/src/storage.rs` | `cargo test -p yet-lsp project_context_inventory`; `cargo test -p yet-lsp project_context_rebuild`; `cargo test -p yet-lsp project_context_status` | Authenticated project-scoped SQLite cache status and explicit rebuild are live. Rebuild inventories only the canonical registered root with bounded ignore, secret, binary, generated, dependency, size, symlink, and containment policy; it stores sorted relative metadata and hashes in an atomic generation without file content. The durable turn-context path is reserved but turn evidence is not persisted. Automatic rebuild/watchers, cache deletion, profile, lexical/symbol retrieval, manifests, turn persistence, partial assistant persistence, Continue, GUI, and host parity remain unsupported. |
