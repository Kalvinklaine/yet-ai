...
| `project_context` | `live_engine` | `apps/engine/src/project_context/`, `apps/engine/src/http/project.rs`, `apps/engine/src/storage.rs` | `cargo test -p yet-lsp project_context_inventory`; `cargo test -p yet-lsp project_context_rebuild`; `cargo test -p yet-lsp project_context_status`; `cargo test -p yet-lsp project_context_profile` | Authenticated project-scoped SQLite cache status, explicit rebuild, and deterministic generation-bound profile are live. Rebuild inventories only the canonical registered root with bounded ignore, secret, binary, generated, dependency, size, symlink, and containment policy; it stores sorted relative metadata and hashes plus bounded structural profile facts in an atomic generation without file content. Profile freshness follows stable project/root cache identity and inventory generation; summaries expose supporting relative refs and hashes, and manifest command hints are filename conventions rather than parsed content. The durable turn-context path is reserved but turn evidence is not persisted. Automatic rebuild/watchers, cache deletion, lexical/symbol retrieval, manifests, turn persistence, partial assistant persistence, Continue, GUI, and host parity remain unsupported. |
...
| Project context index/profile/manifest | `unsupported` — engine status/bootstrap is live, but no host inspector, index, profile, or manifest exists | `unsupported` — engine status/bootstrap is live, but no host inspector, index, profile, or manifest exists | `unsupported` — engine status/bootstrap is live, but no host inspector, index, profile, or manifest exists |
