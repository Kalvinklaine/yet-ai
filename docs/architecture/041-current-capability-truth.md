...
| `project_context` | `live_engine` | `apps/engine/src/project_context/`, `apps/engine/src/http/project.rs`, `apps/engine/src/storage.rs` | `cargo test -p yet-lsp project_context_inventory`; `cargo test -p yet-lsp project_context_rebuild`; `cargo test -p yet-lsp project_context_status`; `cargo test -p yet-lsp project_context_profile`; `cargo test -p yet-lsp project_context_fts` | Authenticated project-scoped SQLite cache status, explicit rebuild, deterministic generation-bound profile, and internal lexical FTS5 retrieval are live. Rebuild inventories only the canonical registered root under bounded deny policy and atomically stores sorted relative metadata/hashes, structural profile facts, and bounded admitted source chunks plus FTS rows for the current generation. The rebuildable cache is local, isolated per project, unencrypted, and permission-restricted where supported; cache deletion is not yet exposed. No indexed text is automatically attached or uploaded. Provider disclosure requires the later explicit plan/review/Send path, which remains unsupported. Automatic rebuild/watchers, public lexical query, symbols, planner/manifests, turn persistence, partial assistant persistence, and Continue remain unsupported. |
