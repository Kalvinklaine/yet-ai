...
| `progress_population` | `local_derived` | `scripts/planner-agent-progress-state.mjs`, `scripts/planner-agent-progress-run.mjs`, explicit callers of `POST /v1/agent-progress/events` | `npm run check:agent-progress`; `npm run smoke:agent-progress` | Populated lifecycle progress is currently produced mainly by scripts, smokes, or explicit developer calls. Normal chat and controlled execution do not yet publish complete lifecycle events automatically; an empty live panel is truthful. |
...
| Progress lifecycle population | `local_derived` — developer/script events | `local_derived` — developer/script events; executors do not publish a complete lifecycle | `local_derived` — developer/script events; executors do not publish a complete lifecycle |
