...
| `project_context` | `live_engine` | `apps/engine/src/project_context/`, `apps/engine/src/http/project.rs`, `apps/engine/src/storage.rs` | `cargo test -p yet-lsp project_context`; `cargo test -p yet-lsp project_context_status` | Only authenticated project-scoped SQLite cache bootstrap and `GET /p/{projectId}/v1/context/status` are live. Status initializes bounded isolated schema metadata without inventorying files and safely refreshes only rebuildable cache state after an authorized project root rebind; the durable turn-context path is reserved but turn evidence is not persisted. Inventory, user-triggered rebuild/delete, profile, lexical/symbol retrieval, manifests, turn persistence, partial assistant persistence, Continue, watchers, GUI, and host parity remain unsupported. |
