...
| `project_context` | `unsupported` | Planned in `042-project-context-platform.md`; Wave 0 schemas and examples under `packages/contracts/` are not runtime code | `npm run validate:contracts`; `npm run validate:docs` | Inventory, SQLite context cache, deterministic profile, lexical/symbol retrieval, manifests, status/profile/rebuild/plan endpoints, turn manifest persistence, partial assistant persistence, Continue, watchers, and host inspector parity are not reachable. Current ADR/schema/example evidence is `fixture_demo` only. Project memory remains the separate `live_engine` explicit-note capability. |
...
| Project context index/profile/manifest | `unsupported` — Wave 0 contracts only | `unsupported` — no host inspector or engine endpoint | `unsupported` — no host inspector or engine endpoint |
