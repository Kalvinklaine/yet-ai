...
| `project_context` | `live_engine` | `apps/engine/src/project_context/`, `apps/engine/src/http/project.rs`, `apps/engine/src/storage.rs` | `cargo test -p yet-lsp project_context_inventory`; `cargo test -p yet-lsp project_context_rebuild`; `cargo test -p yet-lsp project_context_status`; `cargo test -p yet-lsp project_context_profile`; `cargo test -p yet-lsp project_context_fts`; `cargo test -p yet-lsp project_context_symbols` | Authenticated project-scoped SQLite cache status, explicit rebuild, deterministic generation-bound profile, internal lexical FTS5 retrieval, and bounded definition metadata for admitted Rust and TypeScript/JavaScript sources are live. Supported syntax is tree-sitter-backed at higher confidence; malformed supported syntax may use a bounded heuristic fallback recorded with lower confidence. Ranges are validated and use one-based lines plus zero-based UTF-8 byte columns. Rebuild inventories only the canonical registered root under bounded deny policy and atomically stores sorted relative metadata/hashes, structural profile facts, bounded admitted source chunks, symbol provenance/confidence, and FTS rows for the current generation. Symbols are lightweight ranking hints only: consumers must not treat either parser source as compiler-accurate semantics or infer a call/reference graph. The rebuildable cache is local, isolated per project, unencrypted, and permission-restricted where supported; cache deletion is not yet exposed. No indexed text is automatically attached or uploaded. Provider disclosure requires the later explicit plan/review/Send path, which remains unsupported. Automatic rebuild/watchers, public lexical/symbol query, planner/manifests, turn persistence, partial assistant persistence, and Continue remain unsupported. |
...
| `chat_sse_history` | `live_engine` | `apps/engine/src/chat.rs`, chat routes in `apps/engine/src/http/mod.rs` and `apps/engine/src/http/project.rs`, `apps/gui/src/services/runtimeClient.ts` | `cargo test -p yet-lsp chat`; `cargo test -p yet-lsp --test runtime`; `npm run smoke:local` | Local command, SSE, abort, and engine-owned history paths are real. The OpenAI-compatible parser has a known role-only-delta defect until T-159 lands; this status does not waive it. |
...
| Project context status/profile | `live_host` — GUI inspector renders live structural status/profile and requests explicit rebuild; lexical rows are not exposed or attached | `live_host` — packaged GUI uses the same engine path; installed-host parity is not separately proved | `live_host` — packaged GUI uses the same engine path; installed-host parity is not separately proved |
| Project context lexical/planner/manifest | `unsupported` — lexical cache is engine-internal; no query, planner, manifest, or Send integration is reachable | `unsupported` | `unsupported` |
