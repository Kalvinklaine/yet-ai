...
Yet AI needs one product identity contract for its local-first AI coding assistant and IDE agent plugin product. The contract records the product names, package IDs, storage names, plugin IDs, and public URLs that belong to Yet AI.
...
- `strategy`: records the standalone local-first product strategy.
