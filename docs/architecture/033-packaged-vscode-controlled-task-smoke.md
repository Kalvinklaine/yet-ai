...
Current packaged controlled-task recovery evidence is conservative static evidence only. It combines local VSIX archive/content inspection, packaged GUI asset presence checks, bundled identity checks, checksum/copy checks, and documentation/copy assertions for reconnect, stale-state, reload, and update guidance. It may prove that the prepared artifact contains the expected safe labels and that forbidden claims are absent. It does not prove that a user installed the VSIX, launched VS Code, opened the packaged webview, reconnected a real runtime, survived a real webview reload, blocked a live stale host result, reinstalled an updated VSIX, or recovered a controlled task in a running IDE session.

Treat any `npm run smoke:vscode-packaged-controlled-task` result as archive/content/static packaged-GUI evidence plus conservative copy checks. It is not manual install proof, UI automation proof, update-channel proof, crash-recovery proof, production readiness, marketplace readiness, signing/notarization evidence, real-provider evidence, or a broader autonomy approval. Browser remains unsupported for trusted controlled-task execution. JetBrains remains outside this packaged VS Code proof and must stay partial/fail-closed unless a later verified card adds equivalent packaged JetBrains controlled-task evidence. Core chat, provider setup, IDE GUI workflows, and local storage remain local-first BYOK and must not require a hosted Yet AI backend, Yet AI account, managed model gateway, product credit balance, cloud workspace, or provider credentials in automation.

Future manual evidence would need a dated safe-share report from an explicit install-from-file session on a local workstation. That report should identify only safe labels such as OS/architecture, VS Code version family, artifact path family, checksum match status, launch mode, packaged GUI identity/status observations, explicit user gates, reconnect/reload/stale-result observations, deliberate reinstall/update steps, and sanitized final status. It must still omit raw prompts, provider responses, file bodies, diffs, command output, private paths, bridge payloads, tokens, cookies, API keys, screenshots with secrets, and account-private identifiers.

Future automated evidence would require a separately approved harness that launches VS Code or an equivalent extension-host/webview environment, installs or loads the prepared artifact, opens a disposable trusted workspace, drives explicit user-equivalent actions, simulates reload/reconnect/update/stale-result cases, and records only sanitized labels. Such a harness would still need to prove no automatic task start, apply, verification, provider send, hidden read, search, indexing, shell/git/tool execution, package/network mutation, automatic update, cross-project resume, or crash-recovery contract. It must remain local/mock-only unless a later card explicitly approves real-provider dogfood evidence.
