...
Sprint 13 adds a contract-only active-file excerpt bridge action for the same prompt-context boundary. The GUI may request it only through `gui.ideActionRequest` with payload exactly `{ "action": "getActiveFileExcerpt" }`; there is no path input, glob, full-file flag, recursive/index workspace option, provider/model/API-key field, tool, shell, or git field. VS Code and JetBrains are the intended supported hosts after implementation, while browser mode remains unavailable/preview-only. A successful host result may include one `contextAttachment` with `kind: "active_file_excerpt"`, source `vscode` or `jetbrains`, sanitized file metadata, bounded range, bounded text, and an explicit `truncated` boolean. Failed, rejected, or unavailable results must not include the attachment. This excerpt is explicit user context for the currently active editor, previewed/attached for the prompt, and one-shot; it is not assistant-triggered arbitrary file read, full-file retrieval, background scan, workspace indexing, cloud persistence, provider/tool call, shell/git operation, edit authority, or autonomy.
