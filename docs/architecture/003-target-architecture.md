...
- `gui.applyWorkspaceEditRequest`: confirmed edit-proposal request with bounded request correlation, `requiresUserConfirmation: true`, safe summary text, local-only `cloudRequired: false`, and text replacement edits for existing workspace-relative files only. The GUI sends it only after preview/review and explicit user action; VS Code asks the user to confirm again before applying. Browser development mode and JetBrains do not provide an apply path in this MVP.
