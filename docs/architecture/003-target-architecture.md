...
- `scripts/planner-agent-progress.mjs`, `scripts/planner-agent-progress-state.mjs`, `scripts/planner-agent-progress-report.mjs`, `scripts/check-agent-progress.mjs`, and `scripts/smoke-agent-progress.mjs`: local-only sanitized progress reducer/classifier, durable event-state helper, compact CLI reporter, reducer/state checks, and deterministic smoke scenarios for the agent progress observability contract. They do not implement production background agents, runner hooks, task-board integration, git merges, shell authority, tool execution, cloud sync, or workspace mutation.
...
Agent progress observability is currently an MVP contract and local simulator foundation. The implemented scope is strict agent progress event/snapshot schemas with positive and invalid fixtures, a pure reducer/classifier for current progress and stuck/healthy/done states, durable local JSON event-state helpers, a compact CLI reporter, and deterministic smoke scenarios. It is not a production background agent system, real runner integration, real task-board integration, git merge runner, shell authority, tool executor, workspace mutator, hosted service, or cloud sync feature.

The intended future production runner should emit sanitized progress events from explicit lifecycle hooks such as queued, reading context, editing, running commands, waiting for tool output, verifying, finishing, done, failed, or stuck. Those hooks may expose only safe operational summaries: phase/status enums, non-secret run/card/event ids, UTC timestamps, bounded elapsed and heartbeat ages, attempt counts, generic tool kind/label, and short sanitized output-tail summaries. The reducer and CLI reporter then derive current progress and stuck reasons without needing prompts, hidden reasoning, raw logs, or workspace data.
...
The final agent-progress gate context is `npm run check:agent-progress && npm run smoke:agent-progress && npm run check && git status --short`. These commands validate contracts, reducer/state/report behavior, smoke scenarios, and repository hygiene without launching production agents, executing tools, performing git operations, mutating workspaces, calling providers, or requiring a hosted Yet AI backend. The local-first BYOK contract remains unchanged.
