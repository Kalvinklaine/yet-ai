...
S131-C1 classifies the S125-S130 verification command scopes so later wrapper work can surface truthful evidence without changing `npm run check` membership:

| Class | Commands | Evidence role | Setup sensitivity | S131-C2 filter-safe wrapper need |
| --- | --- | --- | --- | --- |
| Lightweight/root-safe | `npm run validate:contracts`; `npm run check:controlled-agent-storage-privacy`; `npm run smoke:controlled-agent-task-harness`; `npm run smoke:controlled-agent-workflow-transcript`; `npm run check` | Contract, metadata, transcript, privacy, and root repository validation | Root Node dependencies and GUI TypeScript resolution for pure-service smokes; no browser, plugin compile, prepared package, provider, hosted service, account, gateway, credit, or cloud workspace | Add wrappers or labels for repo hygiene/root check output, especially `npm run check` and final `git diff --check`/status evidence, so filtered command reports cannot imply unrun chained checks. |
| GUI-dependent | `cd apps/gui && npm run typecheck && npm test -- controlledAgentTaskHarness controlledAgentWorkflowTranscript AgentRunPanel App && npm run build`; `npm run smoke:controlled-agent-vscode-task-e2e` | GUI type/test/build and built-GUI display evidence | Requires `apps/gui` dependencies and current built assets for the built-GUI smoke; may emit non-failing Vite chunk-size warnings | Add a wrapper that runs and reports each GUI substep separately with bounded sanitized output and a clear built-asset prerequisite. |
| VS Code/plugin-dependent | `cd apps/plugins/vscode && npm run compile && npm test -- controlledCommandRun webview controlledVerificationBundle` | VS Code host compile/test evidence for controlled verification and webview surfaces | Requires VS Code plugin dependencies; does not launch VS Code UI or call providers | Add a plugin wrapper that separates compile from test filters and records dependency/setup failures distinctly from product failures. |
| Artifact-dependent | `npm run prepare:vscode-preview`; `npm run smoke:vscode-packaged-controlled-task` | Local dev-preview VSIX preparation and archive/content inspection | Requires root, GUI, engine/package preparation, VS Code plugin dependencies, archive tools, and ignored `dist/plugins/vscode` artifacts | Add packaged wrappers that avoid shell-chain ambiguity, state the prepare-before-smoke dependency, and report missing VSIX/checksum as setup, not passing evidence. |
| Final convergence only | `npm run smoke:controlled-agent-hardening-bundle`; `git diff --check && git status --short` | Fail-fast aggregation and final hygiene snapshot after focused gates | Depends on child gates, and the bundle includes packaged evidence that requires a prepared local dev-preview VSIX | Add final-bundle and hygiene wrappers that print child command outcomes and avoid filtered `&&` chains being cited as complete when a later command did not run. |

S130 final verification status: the S125-S130 local/mock hardening bundle passed in this worktree after restoring local ignored dependency symlinks and preparing the ignored dev-preview VS Code VSIX. Verified commands were `npm run validate:contracts`; `cd apps/gui && npm run typecheck && npm test -- controlledAgentTaskHarness controlledAgentWorkflowTranscript AgentRunPanel App && npm run build`; `cd apps/plugins/vscode && npm run compile && npm test -- controlledCommandRun webview controlledVerificationBundle`; `npm run check:controlled-agent-storage-privacy`; `npm run smoke:controlled-agent-task-harness`; `npm run smoke:controlled-agent-workflow-transcript`; `npm run smoke:controlled-agent-vscode-task-e2e`; `npm run prepare:vscode-preview`; `npm run smoke:vscode-packaged-controlled-task`; `npm run smoke:controlled-agent-hardening-bundle`; `npm run check`; and `git diff --check && git status --short`. Residual status remains dev-preview hardening evidence only: the bundle did not perform manual BYOK dogfood, launch a real VS Code UI session, prove packaged install/reconnect/recovery on a user machine, call real providers, sign or notarize artifacts, publish marketplace packages, approve a release, or widen controlled-agent autonomy. The local GUI build still reports non-failing Vite chunk-size warnings, and built-GUI/package-dependent gates continue to require local dependencies plus prepared ignored artifacts before they can run.
