...
- The functional read-only LSP MVP is available only as engine stdio plus opt-in VS Code local document/completion wiring and spawned stdio smoke coverage. Production AI completions, code-lens/code-vision, provider-backed completions, diagnostics from providers, JetBrains LSP support, full agent autonomy, autonomous file reads/indexing, tasks/knowledge, tool registry execution, shell/tool execution, shell/file mutation, file edits/apply patch, background agent autonomy, production agent progress observability wiring, and integration workflows are not implemented as production features.
