...
- `apps/gui`: React/Vite browser shell with loopback-only runtime client, provider setup/status UI, sanitized model readiness display, local conversation list/create/switch/delete UI backed by engine history endpoints, chat command submission, fetch-streaming SSE parser, runtime error reporting, browser/VS Code/JetBrains logical bridge detection, and confirmed edit proposal preview/review UI with explicit user apply action only.

- `apps/plugins/vscode`: VS Code extension shell with identity-checked manifest and bundled package-route identity, loopback runtime/dev URL validation, SecretStorage-backed manual runtime tokens, packaged GUI asset loading with placeholder fallback, MVP `connect`/`launch`/`auto` runtime modes, webview host, bootstrap/`host.ready` bridge, bounded active editor/selection context snapshot delivery, redacted runtime diagnostics, guarded message handling, a VS Code-confirmed bounded workspace text-edit apply path for GUI-reviewed edit proposals, an off-by-default read-only LSP client that starts a separate no-secret `yet-lsp --lsp-stdio` process for local `file` document sync plus deterministic completion/hover status proofs and bounded document-symbol support, and a local ignored root dev-preview VSIX artifact plus checksum published by `npm run prepare:vscode-preview` under `dist/plugins/vscode/`.
- `apps/plugins/jetbrains`: JetBrains plugin shell with identity checks, Gradle build/tests, loopback runtime/dev URL validation, packaged GUI resource loading with placeholder fallback, MVP `connect`/`launch`/`auto` runtime modes, PasswordSafe-backed local session token, JCEF host boundary, structural JSON bridge validation, bounded active editor/selection context snapshot delivery through the same `host.contextSnapshot` contract as VS Code, strict wrapper/Kotlin execution for the safe read-only/navigation/context controlled IDE actions (`getContextSnapshot`, `openWorkspaceFile`, and `revealWorkspaceRange`) after explicit GUI/user request, a dev-preview confirmed edit apply boundary through the existing `gui.applyWorkspaceEditRequest` / `host.applyWorkspaceEditResult` bridge messages with explicit GUI apply plus IDE/user confirmation for bounded existing workspace-relative text replacements, and a default-disabled JetBrains LSP lifecycle/process foundation that starts a separate no-secret `yet-lsp --lsp-stdio` feasibility shell without native IntelliJ editor/client wiring.
- `packages/contracts`: shared JSON Schemas and examples for current engine and bridge boundaries, including confirmed-only `gui.applyWorkspaceEditRequest` and sanitized `host.applyWorkspaceEditResult`, plus future/simulator-facing no-idle planner scheduler and sanitized agent progress contracts.
...
The bundled `yet-lsp` in current IDE artifacts is a local Cargo build staged from the checkout or CI runner target directory for dev-preview validation. It may be debug or release-profile build output depending on the invoked prepare path, but it is not a signed, notarized, hardened, or provenance-attested production engine. IDE hosts may use that bundled runtime for local `auto`/`launch` previews, but this does not create production-grade runtime lifecycle, platform keychain guarantees, crash recovery, updater behavior, or a stable binary distribution contract.

Future production packaging remains a decision record, not an implemented feature. Before Yet AI can claim production packaging, the following release decision checklist must be documented, implemented, and verified explicitly:

- Marketplace boundary: VS Code marketplace publisher identity, extension ID finalization, marketplace listing copy, icon/screenshot provenance, license/notice material, and marketplace publication workflow; JetBrains Marketplace vendor/plugin identity finalization, plugin verifier policy, update lineage, listing copy, icon/screenshot provenance, license/notice material, and marketplace publication workflow.
- Signing and notarization boundary: signing identities, signing scope for IDE packages and engine binaries, macOS notarization and hardening where applicable, Windows signing where applicable, Linux package trust expectations, timestamping, certificate rotation, failure handling, and evidence retention.
- Engine platform matrix: build profile, target triples, CPU/OS support, per-platform archive layout, bundled runtime naming, keychain availability assumptions, startup smoke coverage, compatibility policy, and verification of bundled runtime provenance for each shipped platform.
- Installer and artifact workflow: whether Yet AI ships IDE marketplace packages only, standalone installers, archive downloads, or a combination; exact artifact names, checksum/signature publication plan, local dev-preview to release migration path, and which generated artifacts remain ignored local outputs.
- Update and rollback boundary: update channel design, version compatibility matrix, rollback policy, emergency withdrawal process, artifact retention period, manifest retention, and whether users can pin or reinstall older approved artifacts.
- Provenance and SBOM boundary: SBOM format, dependency and generated-GUI asset representation, bundled engine binary provenance, build attestation requirements, third-party notices, license material, and publication location for checksum/signature/provenance evidence.
- Release gate boundary: manual dogfood, release-candidate, and production release gates, including which smokes remain local/mock-only, which checks require human approval, which secrets are forbidden in automation, and which step is the final explicit publish approval.

This boundary preserves the local-first BYOK contract. Core chat, provider setup, IDE GUI workflows, local storage, and current dev-preview smokes must not require a hosted Yet AI backend, Yet AI account, managed model gateway, product credit balance, cloud workspace, production account login, real provider credentials in automation, marketplace approval, signing, notarization, or publication.

- VS Code can publish a local ignored dev-preview VSIX artifact at `dist/plugins/vscode/yet-ai-vscode-<version>-dev-preview.vsix` with a `.sha256` checksum for install-from-file smoke testing, and JetBrains has a comparable local ignored ZIP dev-preview artifact at `dist/plugins/jetbrains/yet-ai-jetbrains-<version>-dev-preview.zip`. The `Yet AI IDE Artifacts` GitHub Actions workflow (`.github/workflows/ide-artifacts.yml`) builds per-platform downloadable dev-preview artifacts in a `linux-x64` / `macos-arm64` / `windows-x64` matrix because the JetBrains plugin JAR bundles a native `yet-lsp` runtime. Before upload, CI starts the JetBrains bundled runtime extracted from the built artifact, verifies authenticated `/v1/ping` on loopback, and stops it without launching IntelliJ. Public artifact names are exactly 7 total: `yet-ai-vscode-unzip-first-<os>-<arch>-<sha>` for three platforms, `yet-ai-jetbrains-install-direct-<os>-<arch>-<sha>` for three platforms, and combined `yet-ai-plugin-manifest-<sha>`. The expected GitHub Step Summary can be generated with `artifact:github-summary` and does not add uploaded artifacts. The combined manifest aggregates per-platform commit, checksum, platform, runtime, and artifact metadata. Users must download the artifact whose `<os>-<arch>` suffix matches their local OS/architecture; mixing platforms fails at install time because the bundled engine is platform-specific. The VS Code artifact must be unzipped before installing the inner `.vsix`; the JetBrains direct-install artifact ZIP is selected directly in Install Plugin from Disk. The bundled `yet-lsp` is the dev-preview local cargo build output staged from the runner's `target/<profile>/yet-lsp` (or `yet-lsp.exe` on Windows); it is not a signed or notarized production engine. Do not install an old combined artifact bundle or any generic transport bundle containing both IDE plugins: JetBrains expects a JetBrains plugin ZIP structure and otherwise can fail to load the plugin descriptor. This workflow is local/mock-only validation and artifact upload; it does not publish, sign, notarize, release, upload to a marketplace, call real providers, require provider credentials, or contact a hosted Yet AI backend. No marketplace publication, signing, notarization, production installer, or production release flow is complete.
...
- Privileged IDE actions remain disabled except for the confirmed edit-proposal MVP. That MVP is contract-bounded, local-only, GUI-previewed, explicitly user-requested from the GUI, IDE/user-confirmed before apply, and limited to text replacements in existing workspace-relative files. VS Code remains the reference implementation. JetBrains is allowed only as a dev-preview confirmed edit apply host through the existing `gui.applyWorkspaceEditRequest` / `host.applyWorkspaceEditResult` bridge messages; it must not add new write-capable bridge messages or broader mutation authority. Browser remains preview-only/non-executing for workspace edits. The MVP does not enable autonomous edits, shell/tools/tasks/git, file create/delete/rename, apply-patch, arbitrary file reads/indexing, provider tool execution, provider calls, model-triggered apply, production agent autonomy, or silent workspace mutation. The MVP lifecycle is preview-only until the GUI user clicks `Apply`: the GUI never edits files directly, and an IDE host may apply only after its own user confirmation. The GUI mints the `requestId` on every apply click, so assistant-supplied request ids are not accepted and historical assistant bubbles stay non-runnable; raw proposal JSON is inspect-only/debug while chat bubbles are compact by default. The GUI's pending-state clear is client-side only and does not cancel any open host UI; stale or duplicate `host.applyWorkspaceEditResult` messages are ignored or bounded/cleared; and a non-`applied` status surfaces a short sanitized repair/retry hint with no auto-retry. Model-generated safe-edit output is only a proposal discovery format: a real model may return normal prose or one exact `gui.applyWorkspaceEditRequest` JSON envelope for the GUI to parse, but that envelope must omit `requestId`, require `requiresUserConfirmation: true`, declare `cloudRequired: false`, and contain only bounded text replacements for existing workspace-relative files. Markdown-wrapped JSON, prose around JSON, unknown fields, unsafe paths, create/delete/rename operations, shell/tool/git instructions, and assistant-supplied request correlation are rejected before any runnable apply message exists.
- Controlled IDE actions for this sprint are local-first, bounded, and limited in VS Code and JetBrains to safe read-only/navigation/context actions: `getContextSnapshot`, `openWorkspaceFile`, and `revealWorkspaceRange`. Controlled `getContextSnapshot` returns metadata only (source, active-editor boolean, workspace folder count) with no file path, selected text, or raw content; the separate `host.contextSnapshot` prompt-context bridge may include bounded selected text only for visible opt-in first-message attachment. Open/reveal are navigation only. JetBrains execution uses strict wrapper/Kotlin policy and emits only correlated `host.ideActionProgress` / `host.ideActionResult` after explicit GUI/user request. Browser remains a preview/test fallback and does not execute controlled actions. The `runVerificationCommand` bridge contract is a preview-only controlled action for explicit user-confirmed allowlisted verification command ids (`repository-check`, `gui-app-tests`, and `engine-chat-tests`) with GUI-minted request ids and sanitized result tails; it does not allow free-form shell, args, cwd, env, git, package install, network, provider/model/API-key fields, assistant-triggered runs, browser execution, or workspace mutation. The engine can observe sanitized agent progress/action metadata only and does not execute IDE tools, mutate workspaces, read arbitrary files, call providers for IDE actions, or run shell/git/task/tool operations. Confirmed edit proposals remain the only write path and still require explicit GUI apply plus IDE/user confirmation in VS Code and JetBrains; JetBrains is dev-preview only and browser remains non-executing. No controlled action path adds shell/git/tasks/tool execution, arbitrary file reads/indexing, provider calls, hosted backend/cloud/account requirements, autonomous edits, create/delete/rename/apply-patch behavior, or silent workspace mutation.
- Verification command output can become chat prompt context only through a separate explicit user click after the sanitized host result exists. The chat command contract represents it as a `verification_output` item inside `context.kind = "explicit_context_bundle"` with only `commandId`, `exitCode`, `status`, bounded sanitized `outputTail`, and `truncated`. It rejects raw command strings, cwd, env, shell scripts, provider/model/API-key fields, private paths, secrets, and unknown fields, so the edit→verify follow-up loop remains user-driven prompt context rather than assistant-triggered shell authority.
...
The local project memory MVP is implemented as an explicit, user-created, local-only memory shelf. The engine owns storage under the Yet AI config directory at `project-memory/notes.json`, exposes authenticated loopback endpoints for list/create/get/update/delete/search, validates all payloads, writes notes atomically with private permissions where supported, and rejects unsafe store symlinks. The GUI renders a manual memory panel for create/list/search/delete and explicit attach-to-next-message behavior. The deterministic project-memory smoke verifies manual note creation, literal search, one-shot attach, provider prompt inclusion, accepted-send clearing, deletion, no browser storage persistence, no IDE workspace actions, no non-loopback requests, and no secret/private-path leakage.

A project memory note is manual, user-created, sanitized, and bounded. The runtime shape includes a path-safe note id, title, text, tags, `source: "manual"`, and local timestamps. Runtime and contract validation reject raw provider responses, provider bodies, prompts or command dumps, API keys, OAuth tokens, cookies, credential labels, private absolute paths, credential file names, file bodies, workspace file contents, embeddings, background-index fields, workspace-scan flags, assistant-triggered save markers, browser-storage markers, provider/model/API-key fields, cloud-required flags, unknown fields, and oversized values.
...
Selecting memory as chat context is an explicit user action through the existing `explicit_context_bundle` one-shot prompt-context path. The bundle carries bounded `project_memory` items with note id, title, text, and tags for the next user-approved message only, and the GUI clears the bundle after accepted Send. It does not auto-attach future prompts, persist notes in browser storage, let the assistant create notes, or grant file read, shell, git, provider-tool, provider-call, tool execution, workspace mutation, indexing, or cloud-sync authority. Deleting a memory note means local Yet AI project memory deletion only and does not represent provider-side deletion, enterprise retention, legal hold, backup purge, encrypted sync, or hosted account state.

Runtime/GUI changes for this boundary should run focused engine and GUI tests plus the project-memory smoke. The final Sprint 24 gate is `npm run validate:contracts && export PATH="$HOME/.cargo/bin:$PATH"; cargo test -p yet-lsp && cd apps/gui && npm test -- App && npm run build && cd ../.. && npm run smoke:project-memory && npm run smoke:local && npm run check && git diff --check && git status --short --branch`.

## Active context first-message boundary

The current first-message context capability is intentionally narrow and non-privileged. VS Code and JetBrains hosts may send a bounded active editor/selection snapshot to the GUI through the same strict `host.contextSnapshot` bridge contract. The GUI stores it in React state only, shows a sanitized preview, and includes it in the next accepted chat command only when the user leaves the include toggle enabled. Active context is prompt-only and one-shot: after an accepted send, the GUI clears the attached snapshot and disables the previous include state until the IDE host supplies a fresh snapshot. The engine validates the shape again and prepends an `IDE context` prompt section before the user request when context is present. A chat command may also include an `explicit_context_bundle` for up to 4 user-selected prompt-context items. Active-editor bundle items use sanitized relative/display paths, language id, bounded ordered range, and bounded text. Verification-output bundle items may be attached only by explicit user click after a verification result and carry only sanitized `commandId`, `exitCode`, `status`, bounded `outputTail`, and `truncated`. The bundle has an aggregate selection-text cap and remains one-shot prompt context only. It does not add path input, glob/search, full-file reads, recursive workspace reads, indexing, persistence, provider/model/API-key smuggling, raw command strings, cwd/env/shell scripts, private paths, secrets, tools, git, assistant-triggered request ids, or background context collection.

Sprint 13 implements dev-preview active-file excerpt host support for the same prompt-context boundary in VS Code and JetBrains. The GUI may request it only through an explicit user click that sends `gui.ideActionRequest` with payload exactly `{ "action": "getActiveFileExcerpt" }`; there is no path input, glob, full-file flag, recursive/index workspace option, provider/model/API-key field, tool, shell, or git field. Browser mode remains unavailable/preview-only and does not execute this host action. A successful host result may include one `contextAttachment` with `kind: "active_file_excerpt"`, source `vscode` or `jetbrains`, sanitized file metadata, bounded visible-editor range, bounded text, and an explicit `truncated` boolean. Failed, rejected, unavailable, invalid, unsafe, or stale results must not include or render an attachment, and the GUI exposes a safe pending-state clear/retry path without showing raw payloads. This excerpt is explicit user context for the currently active visible editor, previewed/attached for the prompt, and one-shot; it is not assistant-triggered arbitrary file read, full-file retrieval, background scan, workspace indexing, cloud persistence, provider/tool call, shell/git operation, edit authority, or autonomy.

Sprint 19 defines a contract-only local workspace snippet search boundary for future explicit user selection. The GUI may request it only through an explicit user action that sends `gui.ideActionRequest` with payload exactly `{ "action": "searchWorkspaceSnippets", "query": "literal bounded words" }`; the query is literal, non-empty, bounded, and sanitized. There is no regex/glob/path/cwd/env/shell/git/tool/provider/model/API-key input, no assistant-supplied request id, no indexing flag, and no full-file or recursive workspace read option. A successful host result may include sanitized `queryLabel`, bounded `resultCount`, bounded `snippets` with workspace-relative path, language id, ordered range, bounded text, and `truncated`. This is explicit local snippet discovery for user-selected prompt context only; it does not implement background indexing, arbitrary file reads, provider/tool calls, shell/git/tasks, workspace mutation, cloud persistence, or autonomous context gathering.

Included active context is sent to the configured provider as prompt text. It may contain source code or selected text; selected text is included only when visible to the user and allowed by the attach/policy flow, so users should attach only content they are comfortable sending to that provider and should not include secrets, credentials, private paths, or sensitive data. The first-message context contract is limited to safe labels, bounded relative/display paths, language id, selection range, and bounded selection text; controlled IDE-action `getContextSnapshot` is metadata-only.
...
The manual runner MVP is a narrower progress/UI foundation for a user-driven coding loop. It may show plan, context ready, model response, edit proposed, user applied, verification run, and summarized states through the existing sanitized progress event and snapshot contracts. These states are visible lifecycle markers only: the user still initiates context selection, applies reviewed edits, and triggers allowlisted verification. Manual runner state must not introduce hidden reads, autonomous workspace scans, shell or git execution, tool execution, provider tool calls, auto-apply, auto-run verification, background task-board mutation, hosted services, or workspace mutation authority.

Controlled-autonomy v1 may also show an inert manual-runner plan proposal. This proposal is a display contract only: bounded sanitized title, step labels, rationale, and next-action label. It is not an executable plan object and cannot contain file contents, command strings, git/task/tool/provider fields, raw provider responses, patch bodies, apply requests, hidden reads, verification command strings, cwd/env, auto-run flags, or auto-apply flags. Users must still click separate existing controls for context attachment, edit apply, and allowlisted verification. The proposal contract does not expand autonomy, runner authority, engine commands, bridge messages, tool execution, provider calls, shell access, git operations, workspace reads, or workspace mutation.

Sprint 61 implements the first inert multi-step Agent Run plan preview surface for manual review only. The `agent_run.multistep_plan` contract, GUI parser, Agent Run panel rendering, and built-GUI smoke all treat the preview as sanitized display metadata: title, summary, preview-only step labels, risk labels, expected workspace-relative file labels, and allowlisted verification command-id suggestions. The preview intentionally does not populate safe-edit proposals, bounded-loop metadata, apply requests, verification requests, rollback requests, bridge messages, engine commands, provider calls, hidden file reads, or scheduler instructions. It remains `authority: "metadata_only"`, `cloudRequired: false`, `executionAllowed: false`, and requires manual-action policy flags for no auto-send, no auto-apply, no auto-verification, no auto-rollback, no hidden reads, and explicit user action. It is not multi-step execution, production autonomy, or a background runner; any later send, context selection, edit apply, verification run, repair, retry, or rollback must use separate existing explicit user controls and their existing confirmation gates.

The Sprint 61 focused safety gate is:
...
This gate validates the strict metadata-only contract/parser, GUI inert rendering, built-GUI mock smoke, docs/identity/public hygiene, and diff hygiene. It is local/mock-only, uses loopback runtime/SSE/bridge fixtures, does not call real providers, does not require hosted Yet AI services, accounts, managed gateways, product credits, cloud workspaces, production logins, publishing, signing, or release workflows, and does not execute product shell/git/tool actions.

Sprint 62 keeps the Agent Run second step bounded to prompt drafting after explicit user-run verification. The `agent_run.followup_prompt_draft` contract, `buildVerificationFollowupPrompt(result, mode, context?)` service, Agent Run panel CTAs, and built-GUI follow-up loop smoke treat failed-verification fix prompts and successful-verification follow-up prompts as sanitized composer drafts only. The draft may reference bounded verification metadata, allowlisted command id, status, exit code, truncation flag, safe prior proposal labels, safe plan preview summaries, touched file labels, and explicit user intent. It must not include raw command strings, cwd/env, raw verification output dumps, raw prompts, diffs, patch bodies, file bodies, provider/tool/git fields, secrets, private paths, browser-storage data, hidden reads, or execution metadata.

Sprint 62 is not multi-step execution, automatic repair, automatic retry, automatic rollback, production autonomy, or a background runner. Clicking a follow-up/fix CTA only writes text into the composer and focuses it; it must not call `sendUserMessage`, post bridge apply or verification requests, attach context, change Agent Run readiness, run verification, apply edits, save memory, call providers, request runtime execution-like endpoints, read hidden files, or persist drafts/reports in browser storage. Any actual second model call still requires the user's separate review and manual Send click, and any later context attachment, apply, verification, rollback review, or retry must continue through the existing explicit controls and confirmation gates.

The Sprint 62 focused safety gate is:
...
This gate validates the strict metadata-only follow-up draft contract, sanitized prompt builder, Agent Run CTA wiring, built-GUI mock loop for both failed and succeeded verification outcomes, docs/identity/public hygiene, and diff hygiene. It is local/mock-only and loopback-only; it does not call real providers, launch real IDEs, require hosted Yet AI services, accounts, managed gateways, product credits, cloud workspaces, production logins, publishing, signing, or release workflows, and it does not execute product shell/git/tool actions or grant any autonomous workflow authority.

Sprint 63 keeps the stabilized Agent Run verification matrix explicit and narrow. `npm run check` remains the repository/docs/identity gate and does not run browser or packaged IDE smokes. The S61 and S62 focused gates above are the manual-only Agent Run behavior gates for plan-preview and follow-up/fix drafting changes. The heavier Agent Run safety bundle is still an optional, failure-preserving local/mock review sequence for broader safety audits, not a default check bundle and not evidence of real-provider behavior, production autonomy, marketplace readiness, signing, notarization, or release readiness.

Sprint 64 records the conservative Agent Run readiness taxonomy in `013-agent-readiness-milestone.md`. Current one-step Agent Run is manual local dogfood/dev-preview evidence, S61 plan preview and S62 follow-up/fix drafting are experimental manual-only adjuncts, and browser, VS Code, and JetBrains status must keep their existing authority boundaries: browser cannot apply or run IDE verification, VS Code is a manual dogfood host rather than marketplace/production/autonomy proof, and JetBrains remains experimental manual-only parity evidence. S64 classifies readiness and blocked/deferred capabilities; it does not add controlled autonomy, multi-step execution, hidden reads, provider/tool calling, auto-repair, real-provider CI, production readiness, or release authority.

Sprint 65 adds the coding task session backbone as a GUI-local pure metadata summarizer. `createCodingTaskSessionSnapshot` may summarize an explicit goal, user-selected context bundle metadata, explicit project-memory labels, Agent Run proposal/apply/verification metadata, and GUI-local trace labels into a bounded display snapshot with `authority: "metadata_only"`, `cloudRequired: false`, and `executionAllowed: false`. It keeps all manual-action policy flags conservative: no auto-send, auto-attach, auto-apply, auto-run verification, auto-repair, auto-retry, auto-rollback, hidden file reads, or hidden tool runs. Unsafe fields and raw markers such as prompts, file bodies, diffs, command strings, cwd/env, provider/tool payloads, output dumps, secrets, private paths, and stack traces must be omitted or redacted before display.

S65 is not an endpoint, bridge message, model request, runner instruction, storage contract, provider call, background task session, hidden context gatherer, multi-step execution layer, controlled-autonomy feature, or production support claim. It does not launch runtimes or IDEs, post apply or verification bridge messages, mutate workspaces, persist browser storage, read hidden files, index the workspace, call providers, run shell/git/tools, create memory, attach context automatically, or send chat messages. Users still explicitly select context/memory, review proposals, click Apply through the existing confirmation path, and click allowlisted Verification through the existing command-id-only path.

The focused S65 smoke is:

```sh
npm run smoke:coding-task-session
```

The smoke transpiles and imports the pure GUI service locally, builds representative empty, goal/context, memory, proposal, apply, verification, and trace snapshots, and asserts metadata-only authority, conservative no-auto flags, unsafe marker redaction/omission, and bounded output. It is local/mock-only and side-effect-light: no runtime, provider, browser, IDE, network, shell/git/tool endpoint, workspace mutation, hidden read, or storage authority is exercised. Keep it explicit and outside `npm run check` unless a later approved card changes the default validation bundle.

Sprint 65 final audit status: closed with focused local verification and no high or critical safety finding in scope. The coding task session remains a snapshot/backbone only, not proposal history, guided fix loop, endpoint, bridge message, model request, runner, hidden read, storage persistence, provider call, auto-send, auto-attach, auto-apply, auto-verification, auto-repair, auto-retry, auto-rollback, shell/git/tool authority, controlled autonomy, or production support claim.

Sprint 66 adds proposal history and comparison metadata as a GUI-local display surface over already-known proposal lineage. `createProposalHistory`, `appendProposalHistoryEntry`, `updateProposalHistoryEntry`, and `createProposalHistoryComparisonSummary` may summarize original proposals, follow-up proposals, rejected proposals, user-confirmed apply result metadata, and explicit verification result metadata into bounded sanitized entries and comparison labels with `authority: "metadata_only"`. The policy is intentionally conservative: no apply request, verification request, command run, file read, file write, provider call, storage write, hidden read, runner instruction, scheduler action, repair, retry, rollback, or autonomous step can be derived from history metadata.

S66 proposal history is not proposal persistence, a runtime endpoint, bridge message, model request, provider call, browser-storage contract, apply authority, verification authority, guided fix loop, multi-step execution layer, controlled-autonomy feature, or production support claim. It must not carry raw prompts, diffs, patch bodies, file bodies, command strings, cwd/env, verification output dumps, provider/tool payloads, secrets, private paths, stack traces, browser-storage data, hidden reasoning, or unbounded diagnostics. Users still explicitly review proposals, choose Apply through the existing confirmation path, and choose allowlisted Verification through the existing command-id-only path; history only helps them compare display metadata after those events exist.

The focused S66 smoke is:

```sh
npm run smoke:proposal-history
```

The smoke transpiles and imports the pure GUI proposal history service locally, builds original, follow-up, rejected, applied, verified, and unsafe metadata cases, and asserts metadata-only authority, conservative no-authority flags, bounded output, redaction/omission of unsafe markers, and no raw payload leakage. It is local/mock-only and side-effect-light: no runtime, provider, browser, IDE, network, shell/git/tool endpoint, workspace mutation, hidden read, storage persistence, apply, verification, or autonomy authority is exercised. Keep it explicit and outside `npm run check` unless a later approved card changes the default validation bundle.

Sprint 66 final audit status: closed with focused local verification and no high or critical safety finding in scope. Proposal history remains GUI-local display/comparison metadata only, and the explicit `smoke:proposal-history` gate remains separate from the broader safety bundle and `npm run check`. The audit confirmed no new bridge message, runtime endpoint, browser-storage persistence, hidden read, shell/git/tool/provider authority, auto-send, auto-attach, auto-apply, auto-verification, auto-repair, auto-retry, auto-rollback, proposal persistence, raw payload leakage, or controlled-autonomy behavior was introduced.

The S61/S62 built-GUI smokes share the same local bootstrap expectations: repository dependencies must be installed, `apps/gui` dependencies must be available, and Playwright must have a compatible Chromium browser installed. The smokes build the GUI, serve the built files from loopback, and drive deterministic browser fixtures only; they do not launch VS Code or JetBrains, call configured providers, require hosted services, execute shell/git/tool endpoints, mutate workspaces, or persist raw prompts, diffs, file bodies, secrets, command details, private paths, reports, traces, or follow-up drafts in browser storage. If the Vite build reports chunk-size warnings, those warnings are currently non-failing build diagnostics and should be tracked separately from safety results; they do not change browser, VS Code, or JetBrains authority boundaries and do not imply production packaging or autonomous execution readiness.

Sprint 25 adds the contract/docs foundation for a guided coding task session MVP. The `coding_task_session` state is an inert local display snapshot that stitches the existing user-driven loop together without adding authority: bounded title/goal/session status, selected context refs and counts, memory refs and counts, latest response status, edit proposal status, verification status, and short next-step suggestions. It does not carry raw prompts, provider responses, file bodies, command output, shell/git/tool/task commands, cwd/env, provider/model/API-key fields, secrets, private paths, cloud fields, production account login requirements, or auto-read/search/save/send/apply/run flags. It is not a bridge message, engine command, model request, edit/apply request, verification request, runner instruction, background scan, hosted service contract, or cloud workspace contract. Users still explicitly select context, review responses, apply safe edits through the existing confirmed edit path, and run allowlisted verification through existing user-confirmed controls. The local-first BYOK boundary remains unchanged with `cloudRequired: false` and direct provider access.
...
Native Ollama support is a local provider path under the same engine-owned provider boundary. Provider kind `ollama` uses the default endpoint `http://127.0.0.1:11434`, auth type `none`, and no API-key or provider secret requirement. The local engine calls the user-configured Ollama server directly on loopback/local network according to the provider configuration; the GUI renders only sanitized setup, status, and readiness metadata and must not persist browser-side provider secrets. Ollama model readiness can show chat and streaming ready only when the local server/model is reachable and configured through the engine. This does not add a hosted Yet AI proxy, account login, managed model gateway, product credit balance, cloud workspace, browser credential storage, hidden model calls, provider tool calling, autonomous agents, shell/tool execution, or production-grade tool support.
...
Coding-task UX may later show optional read-only LSP context hints for user review, but those hints are display metadata only. Allowed hint material is limited to already available local LSP diagnostics or document symbols summarized as bounded sanitized severity/count labels, short diagnostic messages, safe workspace-relative file labels, language ids, symbol names, and ordered ranges. Hints must not auto-attach to prompts, change context budgets, affect Send readiness, call providers, request model completions, read files, index the workspace, or become prompt context unless the user explicitly selects an existing allowed one-shot context attachment path. Diagnostic messages and symbol/file labels must stay bounded and reject control characters, private absolute paths, secrets, raw document bodies, stack traces, provider responses, command output, and unbounded logs.
...
JetBrains LSP client wiring is explicitly deferred for this MVP sprint. The current JetBrains plugin has a tested HTTP runtime launcher, JCEF GUI host, sanitized runtime diagnostics, bounded active-context bridge, and a default-disabled lifecycle/process foundation that can start `yet-lsp --lsp-stdio` separately with a no-secret environment for developer feasibility checks. That foundation is verified by Kotlin policy tests and `npm run smoke:jetbrains-lsp-foundation`, which also runs the direct engine stdio smoke. It is not native IntelliJ editor/client LSP support: no IntelliJ LSP client is registered, no editor document synchronization is wired to JetBrains, no JetBrains completion/hover/document-symbol provider is exposed, and no production JetBrains LSP claim is made.

A future off-by-default native read-only path is feasible only after a follow-up proves the selected IntelliJ Platform LSP API, likely `com.intellij.platform.lsp.api` on the current `2024.3.x`/build `243` baseline or an intentional platform/dependency bump, and records any bundled plugin/module dependency needed by Gradle/plugin.xml. That follow-up must start `yet-lsp --lsp-stdio` as a separate project-scoped process from the HTTP runtime, pass no runtime or provider secrets, restrict synchronization to bounded local editor-supplied `file://` documents, add sanitized diagnostics, and verify Gradle plus JetBrains smoke coverage before claiming JetBrains LSP support.
...
- auto-attaching LSP diagnostics or symbol hints to prompts, context budgets, or model requests without explicit user selection;
...
- Logs, traces, progress messages, completion labels/details, hover contents, document symbols, diagnostics, and optional display hints must be sanitized and bounded. They must not include raw unbounded document bodies, provider responses, prompts, secrets, bearer tokens, Authorization values, cookies, API keys, OAuth tokens, PKCE verifiers, private absolute paths, stack traces, command output, or credential filenames.
- Completion/status and hover output must be deterministic and derived from safe synthetic rules or the bounded in-memory document state. Document symbols may derive bounded names/ranges from cached editor-supplied text but must not include raw documentation payloads or unbounded details. Optional diagnostic/symbol hints may only summarize already available LSP display facts for review. These surfaces must not call providers, local model runtimes, hosted services, or tool/integration endpoints.
...
- `host.ideActionProgress` / `host.ideActionResult`: correlated sanitized progress/result messages for explicitly requested read-only/navigation/context controlled actions.
- `host.applyWorkspaceEditResult`: correlated sanitized result for a confirmed edit proposal request.
...
Current accepted GUI-to-host messages:

- `gui.ready`: exact bridge version, bounded optional request id, object payload only, and no unknown top-level fields.
- `gui.ideActionRequest`: explicit GUI/user-requested controlled IDE action request limited to `getContextSnapshot`, `openWorkspaceFile`, and `revealWorkspaceRange`; VS Code and JetBrains may execute these read-only/navigation/context actions, while browser development mode does not execute them.
  `getActiveFileExcerpt` is also allowed only as exact payload `{ "action": "getActiveFileExcerpt" }` for explicit active-editor prompt context; it has no assistant-supplied path, glob, full-file, recursive/indexing, provider/model/API-key, tool, shell, or git parameters.
  `searchWorkspaceSnippets` is also allowed only as exact payload `{ "action": "searchWorkspaceSnippets", "query": "literal bounded words" }` for explicit user-initiated local snippet discovery; it has no regex, glob, path, cwd, env, shell, git, tool, provider/model/API-key, assistant-supplied request correlation, indexing, recursive search, or full-file parameters.
  `runVerificationCommand` is a preview-only controlled action allowed only as exact payload `{ "action": "runVerificationCommand", "commandId": <allowlisted id> }` where the id is `repository-check`, `gui-app-tests`, or `engine-chat-tests`. It requires explicit user confirmation before the GUI sends it, uses only a GUI-minted top-level request id, returns only sanitized result metadata/tail, and rejects command/shell/args/cwd/env/git/package-install/network/provider/model/API-key fields or assistant-supplied request correlation. Browser development mode remains preview-only and non-executing for verification commands.
- `gui.applyWorkspaceEditRequest`: confirmed edit-proposal request with bounded request correlation, `requiresUserConfirmation: true`, safe summary text, local-only `cloudRequired: false`, and text replacement edits for existing workspace-relative files only. The GUI sends it only after preview/review and explicit user action; VS Code remains the reference apply host, and JetBrains may apply only as a dev-preview MVP after its own IDE/user confirmation. Browser development mode remains preview-only/non-executing for workspace edits.
...
Before any disabled privileged action is enabled, Yet AI must add a strict schema, request/response correlation, host origin/source checks where the platform supports them, engine and host policy checks, user confirmation for risky effects, sanitized audit/logging, least-privilege allowlists, and explicit protection against silent workspace mutation. Browser development mode must remain non-privileged. The edit-proposal MVP does not allow autonomous edits, model-triggered apply, provider tool execution, shell/tools/tasks/git, file create/delete/rename, apply-patch, arbitrary file reads/indexing, production JetBrains apply support, or unconfirmed workspace mutation; JetBrains apply is dev-preview only through the existing apply/result bridge contract.
...
Browser-first project isolation v1 is implemented and verified against the contract in `036-browser-project-isolation.md`. One registered project is one canonical root, `/projects` is the default browser home, and every `/p/:projectId/v1/...` request resolves an immutable request-scoped `ProjectContext`; there is no mutable process-global current project. Its engine-owned operational data lives under `projects/<projectId>/...` within the Yet AI config/cache namespaces. This is the authoritative rule for browser-registered project chat, memory, progress, and controlled-run state; the older `.yet-ai` target below remains a possible future explicitly shareable repository-state boundary, not the v1 browser project store. Providers, credentials, runtime authentication, demo mode, and global preferences remain global. The verified browser boundary does not implement future indexes, background indexing, multi-root projects, IDE project binding, daemon or worker ownership, cloud sync, or broader autonomous authority.
...
- Ollama is a local-runtime provider kind with default `http://127.0.0.1:11434` and auth type `none`; readiness is local server/model reachability surfaced through sanitized model metadata, not a cloud login or hosted gateway claim.
...
### S157–S179 correctness and maintainability closure

S157–S179 closed on verified production revision `0c5bffa355c598b1038bd59660e4206c3d890807`. The sequential final matrix passed contracts and repository validation; engine library, runtime, and LSP suites; full GUI tests, typecheck, and production build; VS Code compile and tests; JetBrains module tests; routed project, local-provider, runtime, memory, Agent Run, Demo Mode, conversation-history, and hosted-readiness smokes; and the bounded VS Code file-read, edit, multi-file, and verification executor smokes. The verified revision remained unchanged through those gates, and blocker-only review found no open Critical or High issue.

This closure records verification, not broader product or release readiness. It preserves the capability provenance in `041-current-capability-truth.md`: Browser trusted execution and unsupported host paths remain fail-closed, controlled execution remains bounded explicit VS Code dev-preview behavior, packaging remains unsigned and unpublished, and the local-first BYOK, engine-owned secret, no hidden context, no auto-send, and no autonomous apply boundaries remain unchanged.

S124 useful multi-file controlled agent decision status is partial: S109-S123 provide enough dev-preview evidence to keep hardening the VS Code-first controlled multi-file task flow, but not enough to broaden autonomy or start release work. The evidence-backed pass lanes are bounded metadata/display or local/mock gates such as authority vocabulary, search selection, patch-plan review, recovery guidance, task presets, and packaged beta bundle validation; partial lanes remain provider proposal implementation, VS Code-owned apply/search execution depth, verification execution, real-provider dogfood coverage, packaged install/reconnect/recovery proof, privacy/storage inventory, host parity, and CI stability. Hardening remains the next roadmap lane, with host-owned VS Code execution proof, deterministic CI/quarantine policy, raw-data/privacy checks, repeatable sanitized BYOK dogfood, and packaged install/update/recovery evidence before any broader autonomy or release-oriented work. This does not approve production, release, marketplace publication, signing, notarization, broader autonomy, hidden search, or unattended repair.

S125-S130 hardening convergence has added conservative dev-preview evidence, not production readiness. Current checked-in status is: S125 task-harness metadata and S126 workflow-transcript metadata have deterministic contract/pure-service smoke coverage; S127 storage/privacy has a deterministic checker in `npm run check`; S128 records verification setup boundaries and keeps browser/package-heavy gates explicit; S129 adds packaged VS Code archive/content inspection and built-GUI display smoke evidence when the required local artifacts already exist. Root package scripts expose these gates as `npm run smoke:controlled-agent-task-harness`, `npm run smoke:controlled-agent-workflow-transcript`, `npm run check:controlled-agent-storage-privacy`, `npm run smoke:controlled-agent-vscode-task-e2e`, `npm run smoke:vscode-packaged-controlled-task`, and `npm run smoke:controlled-agent-hardening-bundle`. Only the fast deterministic metadata/storage gates are part of `npm run check`; built-GUI, packaged VSIX, and bundle smokes stay explicit because they can depend on built assets, prepared local dev-preview packages, GUI dependencies, or broader local setup. This evidence keeps the local-first BYOK contract visible and does not launch VS Code, call providers, require hosted Yet AI services, publish packages, sign or notarize artifacts, approve marketplace status, add automatic apply/verification, or broaden autonomy.

The packaged VS Code controlled-task smoke is specifically archive/content/static packaged-GUI evidence plus conservative copy checks for the prepared local VSIX. Its reconnect, stale-result, reload, and update coverage means the artifact and docs contain safe guidance labels and reject forbidden claims; it is not proof of a real VS Code install-from-file session, live UI launch, runtime reconnect, webview reload recovery, stale host-result blocking in a running IDE, VSIX reinstall/update, manual BYOK dogfood, or automated extension-host recovery flow. Future proof for those gaps requires either a sanitized manual install-from-file report or a separately approved local/mock automation harness that installs or loads the artifact, opens a disposable trusted workspace, drives explicit user-equivalent actions, simulates reconnect/reload/update/stale-result cases, and records only safe labels. Browser remains unsupported for trusted controlled-task execution, and JetBrains remains partial/fail-closed for this evidence lane until equivalent packaged JetBrains proof is designed and verified.

S131-S135 classify and wrap the S125-S130 verification command scopes so current reports can cite verifier-backed lanes without changing `npm run check` membership:

| Class | Commands | Evidence role | Setup sensitivity | Current evidence hygiene |
| --- | --- | --- | --- | --- |
| Lightweight/root-safe | `npm run validate:contracts`; `npm run check:controlled-agent-storage-privacy`; `npm run smoke:controlled-agent-task-harness`; `npm run smoke:controlled-agent-workflow-transcript`; `npm run check` | Contract, metadata, transcript, privacy, and root repository validation | Root Node dependencies and GUI TypeScript resolution for pure-service smokes; no browser, plugin compile, prepared package, provider, hosted service, account, gateway, credit, or cloud workspace | `npm run check` remains the default root-safe gate and does not run GUI TypeScript/transpile-dependent controlled-agent smokes directly. Repo hygiene is reported separately through `npm run verify:controlled-agent-repo-hygiene`. |
| GUI-dependent | `npm run verify:controlled-agent-gui`; focused child steps are GUI typecheck, controlled task harness/workflow transcript/AgentRunPanel/App tests, GUI build, and `npm run smoke:controlled-agent-vscode-task-e2e` | GUI type/test/build and built-GUI display evidence | Requires `apps/gui` dependencies and current built assets for the built-GUI smoke; may emit non-failing Vite chunk-size warnings | Use the wrapper for verifier-safe reporting of the GUI lane. Older chained local runs may be cited only as agent-reported local evidence. |
| VS Code/plugin-dependent | `npm run verify:controlled-agent-vscode`; focused child steps are VS Code plugin compile and controlled command/webview/verification bundle tests | VS Code host compile/test evidence for controlled verification and webview surfaces | Requires VS Code plugin dependencies; does not launch VS Code UI or call providers | Use the wrapper for verifier-safe reporting of the VS Code/plugin lane. Older chained local runs may be cited only as agent-reported local evidence. |
| Artifact-dependent | `npm run verify:controlled-agent-packaged`; child steps are `npm run prepare:vscode-preview` and `npm run smoke:vscode-packaged-controlled-task` | Local dev-preview VSIX preparation plus archive/content/static packaged-GUI inspection | Requires root, GUI, engine/package preparation, VS Code plugin dependencies, archive tools, and ignored `dist/plugins/vscode` artifacts; does not install or launch VS Code | Use the wrapper for verifier-safe packaged archive evidence. Treat it as local dev-preview artifact inspection only, not real install-from-file launch, reconnect, reload, update, manual BYOK, provider, signing, notarization, marketplace, or release evidence. |
| Final convergence only | `npm run verify:controlled-agent-hardening-final`; supporting explicit legacy gate `npm run smoke:controlled-agent-hardening-bundle`; repo hygiene child `npm run verify:controlled-agent-repo-hygiene` | Fail-fast final hardening sequence across lightweight check, GUI wrapper, VS Code wrapper, task harness smoke, workflow transcript smoke, storage/privacy checker, packaged wrapper, and repo hygiene wrapper | Depends on child gates, local dependencies, and prepared ignored dev-preview artifacts created by the packaged wrapper; can be slower than `npm run check` | Use the final wrapper as S135 local hardening convergence evidence only. It does not replace manual BYOK dogfood, real IDE install/launch/reconnect evidence, real-provider evidence, or release decisions. |

S130 final verification status: the S125-S130 local/mock hardening bundle was agent-reported as passing in this worktree after restoring local ignored dependency symlinks and preparing the ignored dev-preview VS Code VSIX. The final local evidence included explicit or agent-reported runs for `npm run validate:contracts`, storage/privacy, task harness, workflow transcript, built-GUI controlled-task smoke, `npm run prepare:vscode-preview`, `npm run smoke:vscode-packaged-controlled-task`, `npm run smoke:controlled-agent-hardening-bundle`, `npm run check`, and diff/status hygiene. Because some long `&&` command groups were not accepted as verifier-backed evidence by the command filter, do not cite them as uniformly verified chained commands. Current verifier-backed S131-S135 lanes are `npm run verify:controlled-agent-gui` for GUI typecheck/test/build plus built-GUI smoke, `npm run verify:controlled-agent-vscode` for VS Code compile/tests, `npm run verify:controlled-agent-packaged` for dev-preview VSIX preparation plus packaged archive/content smoke, `npm run verify:controlled-agent-repo-hygiene` for diff/status hygiene, and `npm run verify:controlled-agent-hardening-final` for the complete local hardening sequence. Residual status remains dev-preview hardening evidence only: these wrappers did not perform manual BYOK dogfood, launch a real VS Code UI session, prove install-from-file reconnect/reload/update recovery on a user machine, call real providers, sign or notarize artifacts, publish marketplace packages, approve support readiness, approve a release, or widen controlled-agent autonomy. The local GUI build still reports non-failing Vite chunk-size warnings, and GUI/package-dependent gates continue to require local dependencies plus ignored artifacts before they can run.

S135-C4 records the remaining residual risks in `017-controlled-agent-production-gap-audit.md` without converting every gap into a vague backlog item. The next immediate roadmap candidates are limited to bounded evidence work: a sanitized real VS Code install-from-file launch/reconnect/reload/stale-result recovery report, repeatable sanitized manual BYOK real-provider dogfood reports, and support/export redaction validation for controlled-agent evidence bundles. Signing, notarization, marketplace publication, production release operations, cross-platform update/rollback, long-run performance/reliability, JetBrains parity, Browser unsupported-state proof beyond current labels, and production support/incident response remain recorded residual risks until a later approved lane assigns owners, evidence artifacts, and verification gates. This does not create GitHub Issues, release approval, production support, marketplace readiness, real-provider CI, new host authority, or broader autonomy.
