...
- Controlled IDE actions for this sprint are local-first, bounded, and VS Code-first. The only allowed controlled actions are read-only/navigation/context actions: get context snapshot metadata, open an existing workspace-relative file, and reveal a bounded workspace-relative range. Controlled `getContextSnapshot` returns metadata only, open/reveal are navigation only, and action result metadata must not include raw file contents. Browser and JetBrains do not execute these controlled actions in this sprint; they remain preview-only/unsupported for controlled action execution unless separately planned. The engine can observe sanitized agent progress/action metadata only and does not execute IDE tools, mutate workspaces, read arbitrary files, call providers for IDE actions, or run shell/git/task/tool operations. Confirmed edit proposals remain the only write path and still require explicit confirmation. No controlled action path adds shell/git/tasks/tool execution, arbitrary file reads/indexing, provider calls, hosted backend/cloud/account requirements, autonomous edits, or silent workspace mutation.
