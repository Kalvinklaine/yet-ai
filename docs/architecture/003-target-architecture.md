...
Future provider/capability v2 work is a roadmap track, not an implemented schema or catalog. The current MVP contract is limited to configured-provider model summaries with bounded booleans for chat, streaming, tools, and reasoning, plus readiness states `ready`, `disabled`, `missing_credentials`, `missing_model`, and `unsupported` with short sanitized reasons. A future v2 contract may add capability provenance such as configured, runtime-tested, provider-declared, or local-default; optional context-window classes rather than exact unverified catalog numbers; local-provider availability facts such as local server reachable and configured model present; tested-vs-configured readiness so users can distinguish saved settings from recently verified behavior; and provider-family metadata for display, routing preference, and compatibility explanations. These fields must remain display and routing metadata only. They must not grant shell, git, tool, edit, indexing, provider-tool, autonomous-agent, or workspace-mutation authority, and `tools: true`, reasoning support, provider family labels, or larger context-window classes must never bypass the separate runtime feature flags, bridge policies, schemas, request correlation, and user-confirmation gates. Yet AI must not publish built-in provider catalogs, model snapshots, dynamic model-availability claims, benchmark claims, or provider-family assertions until the engine actually implements and verifies those sources through the local runtime.
