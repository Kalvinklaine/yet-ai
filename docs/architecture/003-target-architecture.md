...
| Artifact-dependent | `npm run prepare:vscode-preview`; `npm run smoke:vscode-packaged-controlled-task` | Local dev-preview VSIX preparation and archive/content inspection | Requires root, GUI, engine/package preparation, VS Code plugin dependencies, archive tools, and ignored `dist/plugins/vscode` artifacts | No dedicated wrapper is currently wired. Report these as explicit local/manual or agent-reported artifact evidence, and treat missing VSIX/checksum as setup, not passing evidence. |
| Final convergence only | `npm run smoke:controlled-agent-hardening-bundle`; `npm run verify:controlled-agent-repo-hygiene` | Fail-fast aggregation and final hygiene snapshot after focused gates | Depends on child gates, and the bundle includes packaged evidence that requires a prepared local dev-preview VSIX | The bundle remains an explicit fail-fast command, not a substitute for individually verified wrapper lanes. Use the repo-hygiene wrapper instead of citing `git diff --check && git status --short` as one verifier-backed chain. |

S130 final verification status: the S125-S130 local/mock hardening bundle was agent-reported as passing in this worktree after restoring local ignored dependency symlinks and preparing the ignored dev-preview VS Code VSIX. The final local evidence included explicit or agent-reported runs for `npm run validate:contracts`, storage/privacy, task harness, workflow transcript, built-GUI controlled-task smoke, `npm run prepare:vscode-preview`, `npm run smoke:vscode-packaged-controlled-task`, `npm run smoke:controlled-agent-hardening-bundle`, `npm run check`, and diff/status hygiene. Because some long `&&` command groups were not accepted as verifier-backed evidence by the command filter, do not cite them as uniformly verified chained commands. Use the S131 wrappers for current verifier-safe lanes: `npm run verify:controlled-agent-gui` for GUI typecheck/test/build plus built-GUI smoke, `npm run verify:controlled-agent-vscode` for VS Code compile/tests, and `npm run verify:controlled-agent-repo-hygiene` for diff/status hygiene. Residual status remains dev-preview hardening evidence only: the bundle did not perform manual BYOK dogfood, launch a real VS Code UI session, prove packaged install/reconnect/recovery on a user machine, call real providers, sign or notarize artifacts, publish marketplace packages, approve a release, or widen controlled-agent autonomy. The local GUI build still reports non-failing Vite chunk-size warnings, and built-GUI/package-dependent gates continue to require local dependencies plus prepared ignored artifacts before they can run.
