...
Future production packaging remains a decision record, not an implemented feature. Before Yet AI can claim production packaging, the following decisions must be documented, implemented, and verified explicitly:

- VS Code marketplace publisher identity, extension ID finalization, marketplace listing copy, icon/screenshot provenance, license/notice material, and marketplace publication workflow.
- JetBrains Marketplace vendor/plugin identity finalization, plugin verifier policy, update lineage, listing copy, icon/screenshot provenance, license/notice material, and marketplace publication workflow.
- Engine binary build profile, per-platform packaging layout, code signing, macOS notarization/hardening where applicable, Windows signing where applicable, Linux distribution expectations, and verification of bundled runtime provenance.
- Installer or install/update format for each platform and IDE family, including whether Yet AI ships IDE marketplace packages only, standalone installers, archive downloads, or a combination.
- Update channel design, compatibility/version matrix, rollback policy, artifact retention, checksum/signature publication, and migration path from dev-preview artifacts.
- Provenance/SBOM/attestation requirements if needed for public distribution, including how generated GUI assets, bundled engine binaries, dependencies, and third-party notices are represented.
- Manual dogfood, release-candidate, and production release gates, including which smokes remain local/mock-only and which human checks are required before publishing.
