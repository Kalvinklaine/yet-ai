...
Current provenance uses ADR 041 vocabulary. The bounded VS Code read, lexical-search, replacement-edit, multi-file apply, and allowlisted verification executors are `live_host` dev-preview paths. GUI reducers and summaries are `local_derived`. Rich controlled-capability values supplied by tests or mock `/v1/caps` responses are `fixture_demo` unless the real engine or matching host supplies them. Browser and JetBrains controlled execution remain `unsupported` where the host matrix below says they fail closed. A visible ready card never promotes fixture metadata to live authority; controlled actions also require matching `live_host` provenance and their existing user, policy, correlation, and confirmation gates.
...
| VS Code | Dogfood-ready dev-preview | VS Code is the primary manual local IDE dogfood host and the only `live_host` executor for bounded controlled read, lexical search, replacement edit, multi-file apply, allowlisted command run, and verification bundle paths. Every path remains explicit, correlated, bounded, and host-validated. This is not marketplace, autonomous, or production-release evidence. |
| JetBrains | Experimental manual-only | JetBrains has `live_host` hosted GUI, active-context, safe navigation, and separate confirmed single-file edit behavior. Controlled-agent read, search, edit, multi-file apply, and verification envelopes remain `unsupported` and fail closed. It is not controlled-execution parity, production parity, or autonomy evidence. |
...
| Explicit Verification | Dogfood-ready dev-preview in VS Code | The controlled path uses the implemented `gui.controlledAgentCommandRunRequest` / `host.controlledAgentCommandRunResult` and verification-bundle envelopes after explicit user approval. It is `live_host` only in VS Code; Browser and JetBrains remain `unsupported`. Fixed command ids and bounded sanitized tails do not grant free-form shell authority. |
...
| S73 controlled workspace readiness | Experimental manual-only | The panel evaluates optional controlled-workspace readiness input as `local_derived` display state. The rich values used by focused GUI tests are `fixture_demo`; the current real engine does not emit the full fixture-shaped controlled caps object. Missing or non-`live_host` provenance keeps actions disabled. The panel cannot create a worktree or grant execution authority. |
| S74 bounded controlled file-read | Mixed provenance | The readiness/evidence panel is `local_derived` over optional inputs that are often `fixture_demo`. Separately, an explicit correlated file-read request has a `live_host` bounded executor in VS Code. Browser and JetBrains controlled file-read envelopes are `unsupported`; no panel or caps fixture grants read authority. |
| S75 allowlisted command-runner evidence | Mixed provenance | Command readiness cards are `local_derived` over optional inputs that can be `fixture_demo`. Separately, fixed allowlisted controlled command and verification requests have `live_host` VS Code executors after explicit approval. Browser and JetBrains execution remain `unsupported`; no free-form shell, args, cwd, env, git, network, package, provider, or tool authority follows. |
...
| S116 real bounded controlled multi-file apply | Experimental manual-only | S116 has a `live_host` VS Code executor for explicit, correlated, user-confirmed replacement edits across a small bounded set of existing safe text files. Browser and JetBrains are `unsupported` and fail closed. S114 plans, S115 review UI, bridge fixtures, and mock smokes alone are not execution evidence. The path adds no create/delete/rename, raw persistence, free-form command, provider/tool, hidden-read, or production-autonomy authority. |
