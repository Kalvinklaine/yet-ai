...
Sprint 75 adds `packages/contracts/schemas/engine/controlled-agent-command-runner.schema.json` as an allowlisted command-id verification foundation only. It records trusted GUI- or host-minted request correlation, explicit user/host confirmation metadata, controlled workspace/run ids, one fixed command id from `repository-check`, `gui-app-tests`, or `engine-chat-tests`, bounded timeout/output-tail limits, and sanitized disabled/blocked/running/succeeded/failed/timed-out/killed result metadata. It is not a free-form shell API and does not define command strings, args, cwd, env, git, network, provider/tool calls, package installs, file reads/writes, auto-run verification, repair, retry, rollback, or autonomous execution. Unknown command ids, assistant-minted request ids, raw command material, unbounded limits, private-path or secret-looking output, and shell/git/network/provider/tool/auto-run authority must fail closed.
