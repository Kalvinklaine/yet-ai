...
S42-C5 final audit status: Sprint 42 remains one constrained, user-confirmed apply-to-verify cycle. The reviewed contract, evaluator, trace families, checkpoint/patch helper, bridge schemas, and smokes add no arbitrary shell/git/tool execution, provider tool calls, new bridge execution message, free-form command/args/cwd/env, auto-send, auto-apply, auto-run verification, auto-rollback, hidden reads/search/indexing/background scans, home/secret/private-path/network/remote-publish authority, or assistant-minted sandbox authority. Apply readiness still requires verified checkpoint metadata, patch plans are preflighted before mutation against explicit safe workspace-relative files, verification remains allowlisted by `commandId` only, trace output is GUI-local/read-only/sanitized, rollback remains explicit and user-triggered through the prior checkpoint boundary, and the local-first BYOK/no-hosted-backend invariant is preserved. Sprint 42 is not production autonomy, marketplace readiness, or an autonomous sandbox agent.


## Sprint 61 multi-step Agent Run plan preview

Sprint 61 starts with `packages/contracts/schemas/engine/agent-run-multistep-plan.schema.json`. This contract defines `agent_run.multistep_plan` as an assistant-authored, GUI-reviewed plan preview only. A valid payload is sanitized metadata: exact version and kind, `authority: "metadata_only"`, `cloudRequired: false`, `executionAllowed: false`, bounded title and summary text, preview-only steps, risk labels, expected touched workspace-relative file labels, allowlisted verification suggestion `commandId` preset metadata, and explicit manual-action policy booleans.

The contract grants no execution authority. It is not a bridge message, runtime endpoint, provider request, model prompt request, tool call, edit proposal, patch payload, verification request, rollback request, scheduler instruction, or autonomous loop. It must not carry raw command strings, args, cwd, env, shell snippets, git metadata, task or package-manager instructions, provider/tool fields, raw diffs, file bodies, raw prompts, provider responses, secrets, private absolute paths, assistant-minted request ids, hidden read/search/index metadata, auto-send flags, auto-apply flags, auto-run verification flags, auto-repair flags, or auto-rollback flags.

Every later action remains a separate explicit user action through an already-designed surface. The user must separately choose whether to send a prompt, review and apply any future bounded edit proposal, run an allowlisted verification command by `commandId`, or request rollback through the existing checkpoint UX when available. The preview cannot start those actions, cannot pre-authorize those actions, cannot lower host policy, and cannot make browser mode mutating. This preserves the local-first BYOK invariant: no hosted Yet AI backend, account, managed model gateway, cloud workspace, or product credit balance is required for the preview.
