...
Active editor/selection context is now a first-message prompt capability, not a login feature. The IDE host may provide a bounded active context snapshot, the GUI previews it and lets the user opt in, and the engine includes it in the provider prompt when present. Because included context is sent to the configured provider, users should not attach selections containing secrets or sensitive private data. This does not enable autonomous file reads, workspace indexing, file edits/apply patch, shell/tool execution, or background agent autonomy.
