...
`npm run artifact:manifest` is the bounded dev-preview provenance/SBOM-style gate for the current preview artifacts. It records the exact source commit, generated timestamp, normalized runner platform, plugin artifact checksums, bundled engine binary name/path/SHA-256, and explicit not-signed/not-published/not-notarized status in the ignored local manifest. `npm run smoke:github-ide-artifacts` validates that embedded manifest after staging split GitHub artifacts. This is deterministic local metadata only: it does not sign, notarize, publish, upload marketplace packages, create attestations, call providers, or include secrets/private paths.

The IDE-bundled `yet-lsp` is staged from the local or CI Cargo build output for the active checkout. It is acceptable for dev-preview `auto`/`launch` validation because the smokes inspect archive layout and loopback startup behavior, but it must not be described as a signed, notarized, hardened, or provenance-attested production engine. Future production work must decide the engine build profile, platform matrix, code-signing/notarization policy, installer/archive layout, update channel, compatibility gates, and any production provenance/SBOM/attestation requirements before publication claims are allowed.
...
The manual runner MVP should reuse this progress-only lane before any production runner authority exists. Its visible loop is plan, context ready, model response, edit proposed, user applied, verification run, and summarized. Implementations may render those steps from sanitized `AgentProgressEvent`, `AgentProgressSnapshot`, or `AgentProgressListResponse` payloads only. The loop is explicitly user-driven: context attachment, edit application, and allowlisted verification remain separate user-confirmed actions. Manual runner state must not carry raw prompts, provider bodies, secrets, private paths, raw file contents, raw command output, command strings, cwd/env, shell/git/tool fields, patch bodies, or source metadata, and it must not auto-read, auto-apply, auto-run verification, call provider tools, mutate workspaces, or require hosted services.

The guided coding task session MVP contract should remain display/state only until separate implementation cards add UI behavior. A `coding_task_session` snapshot may summarize the local user-driven loop across existing boundaries: bounded goal/title/status, selected context references and counts, memory note references and counts, latest model-response status, confirmed-edit proposal status, allowlisted verification status, and short next-step suggestions. It must not be treated as an executable plan, task runner input, model prompt, bridge request, edit/apply payload, verification trigger, search/read/save/send instruction, or scheduler command. Schemas and fixtures reject unknown fields and unsafe text so session state cannot smuggle raw prompts, provider responses, file bodies, command strings, shell/git/tool/task fields, cwd/env, patch material, provider/model/API-key fields, secrets, private paths, auto-read/search/save/send/apply/run flags, production login requirements, or cloud workspace dependencies. Implementation should render it as guidance while preserving separate explicit user clicks for context selection, send, edit apply, and allowlisted verification. The local-first BYOK/no-required-cloud contract stays mandatory.

Manual real-provider guided coding task dogfood is documented separately in `docs/dogfood/real-coding-task.md`. That path is a user-driven local BYOK check only: connect the local runtime, configure and test a provider or local model runtime, draft the task goal, explicitly attach reviewed context/snippets/memory, review the prompt draft, click Send manually, and review the response or bounded edit proposal. The shareable template must stay sanitized with provider kind and non-secret model family/id only, no API keys, runtime tokens, auth codes, cookies, production/default account login evidence, raw private paths, raw prompts, raw provider responses, raw patch bodies, or raw file bodies except bounded reviewed excerpts. CI and smoke automation for guided coding tasks remain mock/loopback-only; they must not call real providers, require hosted Yet AI services, use cloud workspaces, publish artifacts, sign/notarize packages, or claim production release readiness.
...
Model-facing guidance should bias toward one small, bounded replacement rather than broad patch-like output. If JSON is returned, it should be exactly one object with `type`, `version`, and `payload`, no markdown fence or surrounding prose, a short safe human summary, and only existing workspace-relative file text replacements. It must not contain shell, git, test, task, tool, provider, command, cwd, env, auto-apply, auto-run, create/delete/rename/move, apply-patch, absolute/private path, traversal path, URL, secret-like summary, raw prompt, provider response, or unknown fields. If the model cannot express the change safely within that shape, it should return normal explanatory prose instead of a malformed proposal. This keeps no auto-apply and no model-triggered apply intact while still improving real-model proposal quality.
