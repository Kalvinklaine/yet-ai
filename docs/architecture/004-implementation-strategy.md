...
3. `apps/plugins/vscode` provides a VS Code extension shell with identity validation, packaged GUI asset loading, local runtime settings, loopback webview/dev URL policy, MVP `connect`/`launch`/`auto` runtime modes, safe bootstrap, narrow bridge handling, the current VS Code-first controlled IDE action host path for bounded read-only/navigation/context actions, and a local ignored root VSIX dev-preview artifact plus checksum published by `npm run prepare:vscode-preview` under `dist/plugins/vscode/`.
