...
Additional local evidence templates live under `docs/dogfood/`, including the real-provider active-file chat report, the manual real coding task dogfood checklist, the historical one-step Agent Run dogfood checklist, the safe-share Agent Run one-step report template, and the S70 manual Agent Run RC checklist/report template. The experimental Codex-like login checklist at `docs/dogfood/experimental-codex-login.md` now covers sanitized manual labels for form-urlencoded exchange/refresh, safe account/model metadata, Responses SSE first chat, one small VS Code controlled task, and reload/reconnect observations; `npm run dogfood:experimental-codex-login-report` checks the template/report shape, and `npm run smoke:experimental-codex-login` plus `npm run smoke:experimental-codex-controlled-task` provide deterministic local/mock closure evidence without real provider credentials. S91-S94 controlled agent dev-preview alignment lives at `docs/dogfood/s91-controlled-agent-dev-preview.md`: it keeps the S90 `partial` decision clear while providing a safe manual VS Code-first dogfood checklist for explicit Start/Stop, one bounded read, one bounded replacement edit, one allowlisted verification command id, at most one user-confirmed repair attempt, host limitations, sanitized reporting, and exact local smoke commands. Browser remains preview-only/unsupported for trusted workspace execution, JetBrains remains partial/fail-closed for controlled execution, and the checklist keeps local-first BYOK visible with no hosted Yet AI backend, account, managed gateway, product credit balance, cloud workspace, production autonomy, real-provider CI, release, or marketplace claim. S92 sanitized report evidence is covered by `npm run smoke:controlled-agent-dev-preview-report` and included in `npm run smoke:controlled-agent-dev-preview`; it is pure local/mock report metadata only, not provider, runtime, network, git, package, shell, broad workspace, production autonomy, real-provider CI, release, or marketplace evidence. S94 closes the S91-S94 roadmap as documentation and wording-audit evidence only: S91 status service/UI/audit, S92 sanitized report, S93 dogfood fixtures/smoke/runbook, and S94 cross-IDE limitation copy remain bounded to VS Code-first explicit-control dev-preview evidence, with Browser preview-only/unsupported and JetBrains partial/fail-closed. S96 uses the same dogfood document for the one-step useful-run docs contract: explicit Start in VS Code, one bounded controlled read, one bounded replacement edit, one allowlisted verification command id, and one sanitized terminal report, with no production autonomy, hidden reads/search/indexing, arbitrary shell, multi-step autonomy, auto-repair, Browser trusted execution, JetBrains controlled parity, release, or marketplace claim. S106 adds `npm run dogfood:controlled-beta-report` as a deterministic local controlled dev-preview beta report template/checker; it validates sanitized Start/Stop, bounded read/edit, allowlisted command-id, one repair-attempt state, result, and follow-up labels while rejecting secrets, raw prompts, raw file bodies, raw diffs, raw command/stdout/cwd/env material, private paths, provider or bridge payloads, browser-storage dumps, hidden authority claims, hosted backend/account/credit requirements, and production, release, or marketplace claims. S65 coding task session backbone status is documented in `architecture/003-target-architecture.md`: it is a GUI-local metadata summarizer over explicit goal/context/memory/proposal/apply/verification/trace inputs only, not an endpoint, bridge message, model request, runner, autonomy layer, hidden read, provider call, or workspace mutation path. The focused local/transpiled service smoke is `npm run smoke:coding-task-session`; it uses no runtime, provider, browser, IDE, shell/git/tool, storage, or network authority and remains explicit rather than part of `npm run check`. S120 controlled recovery matrix UX is documented in `architecture/025-controlled-agent-recovery-matrix.md`: Agent Run and Controlled Agent Run panels render visible stop/stale/disconnect/provider-timeout/edit-mismatch/verification-failure/repair-exhausted/rollback-review/unsupported-host guidance as sanitized, bounded, manual-only display metadata. The focused local/mock smoke is `npm run smoke:controlled-agent-recovery-matrix`; it exercises every recovery state plus unsafe automatic recovery, stale acceptance, raw/private/secret, and unsupported-host overclaim fail-closed paths, with no runtime/provider/bridge/apply/verification/command/git/tool/network/storage/workspace mutation authority. S119-C3 two-step controlled run UI evidence is documented in `architecture/024-controlled-agent-two-step-run.md` and covered by `npm run smoke:controlled-agent-two-step-run`: it renders planning/review/execution/apply/verification/follow-up state as sanitized staged evidence with visible user gates, while blocking missing-gate, stale/duplicate, unsafe/raw metadata flows and adding no auto-send, hidden context acquisition, bridge post, apply, verification, repair, storage, provider/tool, command/git/package/network authority, production-autonomy claim, Browser trusted execution, or JetBrains parity claim. S112 closes S109-S112 as the controlled lexical search Phase 1 foundation in `architecture/019-controlled-agent-explicit-lexical-search.md` and `architecture/013-agent-readiness-milestone.md`: lexical search is explicit, bounded, VS Code-executed only where verified, and user-selected in GUI metadata before any later S113 provider proposal work. The focused smoke is `npm run smoke:controlled-agent-search-selection`; it is deterministic local/mock evidence over GUI pure services, verifies safe selection plus unsafe/private/secret/raw omission and bounded summaries, and adds no hidden indexing/search, browser storage raw persistence, auto-send, auto-attach, provider call, apply, verification, command/git/package/network/tool authority, Browser trusted execution, or JetBrains parity claim. S121 task presets are documented in `architecture/026-controlled-agent-task-presets.md`: the Agent Run UI renders five safe preset buttons and visible user-reviewed draft guidance only. The focused smoke is `npm run smoke:controlled-agent-task-presets`; it covers every preset plus unsafe/no-auto behavior and adds no hidden read/search/indexing, auto-send, auto-attach, provider call, apply, verification, browser-storage persistence, arbitrary command/git/network/tool authority, Browser trusted execution, or JetBrains parity claim. S122 adds `docs/dogfood/controlled-agent-real-provider-matrix.md` and `npm run dogfood:controlled-agent-real-provider-matrix` as a manual local BYOK real-provider matrix template/checker for provider/runtime families and task presets. S144 evolves that template into the v2 repeatable sanitized BYOK evidence contract: copied ignored-local reports use safe run-group and repeated-run labels, planned/completed counts, separate provider and runtime families, host/artifact labels, explicit context/proposal/apply/verification outcomes, failure/recovery paths, reconnect/reload/stale outcomes, and a sanitized final assessment. Comparable attempts hold preset, context shape, host/artifact, and verification shape constant or record a safe comparability note; tracked rows remain `not run` and no real observation is implied. The matrix covers successful small tasks, failed verification plus manual follow-up, provider timeout/error, reconnect/reload, stale/duplicate rejection, unsupported host limitations, and multi-file patch blocking. It rejects credentials and account identifiers, raw prompts/responses/file bodies/diffs/replacements, raw command material/output, private paths, provider payloads, bridge dumps, hosted backend/account/gateway/credit/cloud workspace requirements, real-provider CI, production, release, marketplace, and publication claims. Browser remains preview-only/unsupported for trusted workspace execution and JetBrains remains partial/fail-closed. Manual evidence complements deterministic tests; it does not replace schemas, fixtures, smokes, repository checks, or human release decisions. S123 adds `architecture/027-controlled-agent-task-level-beta-gate.md` and `npm run dogfood:controlled-agent-task-beta-report` as a deterministic template/checker for packaged VS Code task-level dev-preview beta evidence. It ties together task preset selection, explicit context/search selection, provider proposal review, multi-file patch-plan review, explicit apply when safe, allowlisted verification bundle, follow-up/recovery guidance, and sanitized final report/export/history evidence while rejecting secrets, raw prompts/responses/file bodies/diffs/replacements/commands/output/private paths/provider payloads/bridge dumps/browser-storage dumps, hidden authority, hosted backend/account/gateway/credit/cloud workspace requirements, and production/release/marketplace/publication/signing/notarization/real-provider CI claims. S123-C2 adds `npm run smoke:controlled-agent-task-beta-bundle` as the deterministic packaged/dev-preview task-level bundle: it fail-fast runs the existing local/mock child gates for controlled search/selection, task presets, multi-file patch-plan review, two-step apply/verification/follow-up metadata, recovery matrix guidance, and the task beta report template/self-test. The bundle output is sanitized and local/mock-only: no real provider credentials, real-provider CI, hosted service, account, managed gateway, product credit, cloud workspace, raw prompt/response/file/diff/replacement/command/output/private path, provider payload, bridge dump, browser-storage dump, production approval, release approval, marketplace approval, signing, notarization, or publication claim. This remains dev-preview gate evidence only; it does not implement packaged automation or approve production, release, or marketplace status. S69 task memory suggestions are documented in `architecture/013-agent-readiness-milestone.md`: they are GUI-local display metadata over already-listed project memory note metadata, and a note is added to the one-shot bundle only after an explicit user Attach click through the existing project-memory path. The focused local/transpiled service smoke is `npm run smoke:task-memory-suggestions`; it covers suggested/stale/unsafe/already-attached/unrelated statuses, no auto-attach, sanitized session/trace labels, runtime-context label exclusion, browser-storage cleanliness, and no automatic send/search/save/provider/bridge/workspace mutation. S71 multi-step task timeline status is documented in `architecture/012-coding-session-trace.md` and `architecture/013-agent-readiness-milestone.md`: it is read-only sanitized metadata UX over already-known GUI state, not an execution engine, not autonomy, and not raw-data or browser-storage persistence. The exact focused smoke is `npm run smoke:multi-step-task-timeline`; T-315 delivered this replacement smoke after the failed T-312 attempt, so T-312 is not successful evidence. Focused implementation checks are `cd apps/gui && npm test -- multiStepTaskTimeline MultiStepTaskTimelinePanel App` and `npm run check`. S64 readiness status is canonical in `architecture/013-agent-readiness-milestone.md`: current Agent Run dogfood is manual local dev-preview evidence only, S61/S62 are manual-only adjuncts for inert plan preview and draft-only follow-up/fix prompts, and S64 classification does not enable autonomy. The deterministic mock-only built-GUI validation for the safe-share Agent Run path is `npm run smoke:agent-run-dogfood`; it uses loopback-only runtime/host mocks and sanitized evidence, not real-provider CI. The inert multi-step Agent Run plan preview smoke is `npm run smoke:agent-run-multistep-plan`; it covers valid and rejected preview metadata with no automatic apply or verification bridge messages and no browser-storage leakage. The bounded Agent Run second-step follow-up loop smoke is `npm run smoke:agent-run-followup-loop`; it covers failed-verification fix drafts and successful-verification follow-up drafts as composer/focus-only actions with no automatic send, apply, verification, repair, rollback, attachment, or browser-storage persistence. The S66 proposal history/comparison smoke is `npm run smoke:proposal-history`; it transpiles the pure GUI proposal history service locally and asserts original, follow-up, rejected, applied, and verified proposal lineage remains bounded, redacted, metadata-only, and display-only with no apply, verification, runner, storage, browser, IDE, runtime, provider, shell/git/tool, or autonomy authority. These focused gates are separate from the heavier safety regression bundle. Run `npm run smoke:agent-run-safety-bundle` only before merging broad Agent Run safety-boundary changes when you need the curated model proposal, checkpoint readiness, apply, verification, S61 plan preview, and S62 follow-up-loop gates in one fail-fast optional pass.
S141 adds `dogfood/vscode-installed-controlled-task.md` as the sanitized manual/local report contract for a real VS Code install-from-file controlled-task session. Use it to record safe artifact-family, host/version/OS, disposable trusted workspace, install/launch, runtime readiness, reconnect, webview reload, stale-result blocking, deliberate reinstall/update, and terminal outcome labels. It explicitly separates live manual observation from archive inspection, preserves explicit user gates and local-first BYOK, keeps Browser unsupported and JetBrains partial/fail-closed, and excludes secrets, raw prompts/responses, file bodies/diffs/replacements, command material or output, provider/bridge payloads, private paths, unsafe screenshots, and production/release/marketplace/signing claims. Keep completed reports in ignored local evidence locations unless a task explicitly requests a reviewed sanitized excerpt.

## Project Command Center MVP

The Project Command Center MVP is implemented on browser `/projects` project homes and the trusted-host Current Workspace Dashboard. It combines verified runtime/provider readiness with safe recent-chat, memory-summary, and active-work metadata. Memory selection is explicit, bounded, in-memory, and project/chat/generation-bound; chat receives it as a visible unsent one-shot attachment. Start new isolates creation and accepts the engine-issued chat id. Resume and same-project Agent progress are explicit actions. Manual Send is required, rejected handoff retains selection, and accepted Send clears it.

`npm run smoke:project-command-center` is the real-engine/built-GUI loopback proof for this journey. It uses an isolated temporary project and Demo Mode; it is not real-provider CI or installed-IDE evidence. The milestone adds no hidden injection, auto-send, autonomous work resume, Browser trusted edit execution, hosted backend requirement, durable raw context storage, production/release/marketplace/signing claim, or JetBrains parity expansion.
...
- `architecture/012-coding-session-trace.md` — GUI-local read-only coding-session trace model, event families, sanitization rules, in-memory scope, and non-authority boundaries.
  ...
  Sprint 44 deterministic model-proposal coverage is available with `npm run smoke:model-proposal-agent-run`; it exercises the pure one-step prompt/proposal services with valid and rejected mock assistant responses, proves prompt explicit-context-only wording, strict safe-edit recognition, fail-closed malformed/unsafe proposal handling, metadata-only mock provider proposal adoption, sanitized repair guidance, no browser storage, and no auto apply or auto verification.
...
- `architecture/014-controlled-run-history.md` — S99 controlled run history contract for sanitized local run metadata only: run IDs, timestamps, host/readiness/phase/result labels, counters, artifact references/checksums, and safe summaries. It is design-only until later implementation work and explicitly forbids raw prompts, file bodies, replacement text, diffs, command material, provider payloads, private paths, task execution, production autonomy, release, or marketplace claims.
- `architecture/015-controlled-run-explicit-context.md` — S100 explicit multi-file/fragments context contract for controlled runs. It permits only user-selected bounded files/fragments with visible previews and one-shot inclusion, and forbids hidden workspace reads, search, indexing, automatic memory/context injection, raw full-file persistence, production autonomy, release, or marketplace claims.
...
- `architecture/017-controlled-agent-production-gap-audit.md` — S107 production-readiness gap audit scaffold for the controlled-agent path. It records evidence needed before any production-like autonomous-agent claim across security, authority, UX and user controls, CI/CD and flakes, host parity, recovery UX, persistence/privacy, performance, packaged evidence, provider dogfood readiness, and observability while keeping current status dev-preview only.
- `architecture/018-controlled-agent-authority-registry.md` — S109 controlled-agent authority registry v1 contract foundation. It defines the fail-closed vocabulary for file read, lexical search, edit/apply, verification command ids, provider proposal use, memory attachment, sanitized history/report/export/observability writes, host actions, and unsupported privileged operations without adding runtime, bridge, provider, search, edit, command, release, or marketplace authority. Run `npm run check:controlled-agent-authority-registry` after changing this registry document, its safe fixture, or its invalid fixtures; the deterministic local check is also included in `npm run check`.
- `architecture/019-controlled-agent-explicit-lexical-search.md` — S110 explicit lexical search contract and S111 VS Code-only executor evidence for GUI/user-minted bounded literal search metadata with sanitized snippet results. Browser remains unsupported, JetBrains remains fail-closed, and there is no hidden indexing, broad scan, regex/glob, provider/tool, automatic search, production autonomy, release, or marketplace authority. Run `npm run smoke:controlled-agent-lexical-search` after changing the S110 lexical-search contract fixtures or GUI pure service; it is deterministic local/mock evidence only and is included in `npm run check`. Run the focused executor smoke with `npm run smoke:controlled-agent-real-lexical-search` after changing the VS Code executor, bridge wiring, or S111 docs; it is not part of `npm run check` because it recompiles/imports the VS Code plugin and is intended as a targeted S111 gate.
- `architecture/020-controlled-agent-search-informed-proposal.md` — S113 search-informed provider proposal contract for metadata-only provider proposals that may cite only explicit user-selected controlled search/context evidence labels, ranges, hashes, and sanitized summaries. It adds no provider/runtime implementation, provider tool calling, hidden context expansion, command execution, auto-apply, automatic follow-up, production autonomy, release, or marketplace authority. Run `npm run validate:contracts` after changing the S113 schema or fixtures.
- `architecture/021-controlled-agent-multifile-patch-plan.md` — S114 bounded review-plan contract plus the later S116 explicit VS Code-only bounded multi-file replacement executor boundary. S114/S115 metadata and fixtures are not authority; S116 is `live_host` dev-preview behavior only after a GUI/user-minted correlated request, explicit confirmation, and host validation. Browser and JetBrains execution remain `unsupported`. Run `npm run validate:contracts` for contract evidence and `npm run smoke:controlled-agent-real-multifile-edit` for focused executor evidence.
- `architecture/022-controlled-agent-verification-bundles.md` — S117 verification bundle contract and bounded VS Code started-run integration for explicit user-approved sequences of fixed allowlisted command ids. Fixtures are `fixture_demo`; the VS Code executor is `live_host` dev-preview behavior. Browser and JetBrains execution remain `unsupported`. The path forbids free-form command strings, args/cwd/env/shell, git/package/network/provider/tool fields, raw output dumps, private paths, secrets, automatic run claims, and production/release overclaims.
- `architecture/023-controlled-agent-verification-followup.md` — S118-C1 verification follow-up contract for metadata-only proposals derived from sanitized S117 bundle summaries plus an explicit user-selected next action. It permits command ids/labels, statuses, bounded output summaries/hashes/counts, and error categories while forbidding raw stdout/stderr, command strings, cwd/env, private paths/secrets, provider-tool calls, hidden context gathering, automatic provider send, automatic repair, auto-apply, and production/autonomy overclaims. Run `npm run validate:contracts` after changing the S118-C1 schema or fixtures.
- `architecture/024-controlled-agent-two-step-run.md` — S119-C1 two-step controlled run state contract for metadata-only proposal/review and explicit user-gated verification/follow-up phases. It forbids auto-start, auto-verify, provider/tool calls, free-form commands, hidden reads/search, broad mutation authority, stale identifiers, raw payload persistence, unbounded steps, and production/autonomy overclaims. Run `npm run validate:contracts` after changing the S119-C1 schema or fixtures.
- `architecture/025-controlled-agent-recovery-matrix.md` — S120 recovery matrix v1 contract for visible stop, stale result, disconnect/restart, provider timeout, edit mismatch, verification failure, repair exhaustion, checkpoint review, and unsupported-host states. It is docs/contracts/fixtures only and forbids hidden retry, automatic rollback, hidden repair, unbounded attempts, stale result acceptance, raw output/private path/secret persistence, runtime mutation, unsupported-host overclaims, and production/release claims. Run `npm run validate:contracts` after changing the S120 schema or fixtures.
- `architecture/026-controlled-agent-task-presets.md` — S121-C1 real coding task preset taxonomy and contract for safe VS Code-first task starters. It permits bounded preset metadata and explicit user gates while forbidding unsafe authority, hidden search, raw persistence, broad workspace mutation, free-form commands, production claims, or bypassing search/patch/verification review flows. Run `npm run validate:contracts` after changing the S121-C1 schema or fixtures.
- `architecture/027-controlled-agent-task-level-beta-gate.md` — S123 packaged VS Code task-level beta gate and sanitized report contract for useful controlled tasks. It requires task preset, explicit context/search, provider proposal review, multi-file patch-plan review, explicit apply when safe, verification bundle, follow-up/recovery, and sanitized final report/export/history evidence while remaining dev-preview report/gate documentation only. Run `npm run dogfood:controlled-agent-task-beta-report -- --check-template` and `npm run dogfood:controlled-agent-task-beta-report -- --self-test` after changing the S123 template or validator; both are included in `npm run check`. Run `npm run smoke:controlled-agent-task-beta-bundle` for the packaged/dev-preview task-level bundle that fail-fast orchestrates the existing local/mock child gates for controlled search/selection, task presets, patch-plan review, staged apply/verification/follow-up metadata, recovery guidance, and report validation.
- `architecture/028-useful-multifile-controlled-agent-decision.md` — S124 useful multi-file controlled agent evidence decision summary and S124-C3 final verification/postmortem. It classifies S109-S123 authority registry, explicit search/selection, provider proposal, multi-file patch review/apply, verification/follow-up, two-step state, recovery, task presets, real-provider dogfood, and packaged beta gate evidence as pass/partial/blocked, preserves non-goals, records the overall decision status as partial, documents sanitized final verification results, and recommends hardening before broader autonomy or release work. Run `npm run check:useful-multifile-agent-decision`, `npm run check`, and `git diff --check` after changing this decision record.
- `architecture/029-vscode-controlled-task-harness.md` — S125 VS Code-first controlled task harness contract for one deterministic dogfood hardening journey. It defines metadata-only task preset, explicit context/search selection, proposal review, patch-plan review, explicit apply, verification bundle, follow-up/recovery, stale or duplicate correlation fail-closed states, unsupported Browser, partial/fail-closed JetBrains, and no production/release/marketplace claims. Run `npm run validate:contracts`, `npm run check`, and `git diff --check` after changing the S125-C1 schema or fixtures.
- `architecture/030-controlled-agent-workflow-transcript.md` — S126-C1 sanitized controlled workflow transcript/export contract for display-only evidence across explicit user gates, stage transitions, context/search labels and counts, proposal, patch-plan, apply, verification, follow-up, recovery, omissions, unsafe statuses, and final task evidence. It forbids raw prompts, files, diffs, replacements, commands, output, provider payloads, private paths, secrets, bridge dumps, browser-storage dumps, authority to act, and production/release/autonomy overclaims. Run `npm run validate:contracts` after changing this schema or its fixtures.
- `architecture/031-controlled-agent-storage-privacy-inventory.md` — S127-C1 controlled-agent storage/privacy inventory for browser storage, GUI in-memory and persisted state, engine/runtime storage and logs, VS Code and JetBrains host state, dogfood/report/export/docs/fixtures, package artifacts, retention/deletion gaps, forbidden raw-data categories, and future evidence requirements. It is documentation-only hardening evidence and does not implement storage changes, deletion controls, export behavior, package cleanup, production/release readiness, marketplace readiness, or broader autonomy. Run `npm run check` and `git diff --check` after changing this inventory record.
- `architecture/032-controlled-agent-verification-setup-hardening.md` — S128-C1 design for controlled-agent GUI-transpile smoke dependency and worktree verification setup hardening. It defines the supported project-local dependency layout, TypeScript resolution order, actionable missing-dependency diagnostics, `npm run check` versus explicit heavy-smoke boundaries, CI/worktree setup notes, and a Wave 2 implementation plan without changing scripts or adding install automation. Run `npm run check` and `git diff --check` after changing this design record.
- `architecture/033-packaged-vscode-controlled-task-smoke.md` — S129-C1 packaged VS Code controlled-task smoke design for local install-from-file dev-preview evidence. It defines the `npm run prepare:vscode-preview` artifact source, local VSIX install assumptions, visible controlled-task surface checks, reconnect/stale/update/reload guidance, sanitized evidence/reporting rules, and explicit non-goals while preserving local-first BYOK boundaries and avoiding Browser or JetBrains promotion. Use `npm run smoke:vscode-packaged-controlled-task` for this controlled-task packaged evidence after preparing the local VSIX; use `npm run smoke:vscode-installable` separately for generic VSIX/archive layout and install-from-file readiness. Run `npm run check` and `git diff --check` after changing this design record.
- `architecture/034-experimental-codex-login-dogfood-audit.md` — S136-S140 current-flow audit, dogfood gap matrix, and closure boundary for the experimental Codex-like login path. It maps start/status/exchange/disconnect, form-urlencoded exchange/refresh, safe account/model metadata, dedicated Responses SSE first chat, engine secret custody and refresh behavior, GUI and VS Code visible flow, mock-only versus real/manual boundaries, reference-derived UX requirements, severity-ranked gaps, deterministic closure evidence via `npm run smoke:experimental-codex-login` and `npm run smoke:experimental-codex-controlled-task`, and the current S141-S142 not-recommended decision while preserving experimental, private-endpoint-style, non-default, not-official-OAuth, and not-production-ready wording. Run `npm run check` and `git diff --check` after changing this audit.
- `architecture/035-provider-oauth-architecture.md` — ADR for the approved reference-informed provider OAuth framework before code migration. It compares Yet AI's target architecture with inspected reference patterns for PKCE browser callback, future device flow, refresh handling, and MCP OAuth route separation; defines the provider-auth state machine, adapter SPI, session registry, persistence model, callback-server ownership, `/v1/provider-auth/...` compatibility, migration/rollback sequencing, local-first secret custody, and the policy gate that keeps production/default OpenAI account login unavailable until separately approved. Run `npm run check` and `git diff --check` after changing this ADR.
  Current implementation notes in this ADR document the framework/adapters split, canonical stored-auth classifier and refresh quarantine, `provider_auth/mod.rs` compatibility façade role, fixed engine-owned callback listener, persisted-registry callback authority, narrow public status/source projection, safe diagnostic taxonomy, test-only device-flow proof, and the provider-auth cutover/debug runbook. The current implemented surface is API-key fallback, mock OAuth harness, experimental OpenAI/Codex-like browser PKCE/manual/callback path, adapter dispatch/session registry, and test-only device-flow proof; public device-flow `userCode` and complete verification URI UX remain target/future, not production support. For final provider-auth cutover work, inspect port `1455` without stopping an unconfirmed runtime, then run `git diff --check`, `cargo check -p yet-lsp`, `cargo test -p yet-lsp --lib`, `cargo test -p yet-lsp provider_auth`, `cargo test -p yet-lsp --test runtime`, `npm run smoke:local`, `npm run check`, and `cd apps/gui && npm test && npm run typecheck`. The complete release gate must run on the exact merged `main` HEAD.
- `architecture/036-browser-project-isolation.md` — authoritative implemented and verified browser-first project isolation v1: opaque project identity, private canonical-root registry, `/projects` hub and scoped route contracts, request-scoped `ProjectContext`, config/cache namespaces, bounded directory discovery, migration/rollback, strict switching, safe hub fields, and explicit non-goals. Its focused real-engine/built-GUI two-project gate is `npm run smoke:browser-project-isolation` and requires Playwright Chromium. Run `npm run validate:docs`, `npm run validate:hygiene`, and `git diff --check` after changing it.
- `architecture/039-controlled-agent-support-export.md` — S146 strict metadata-only controlled-agent support/export evidence contract. It defines engine-sanitized local custody, user-managed local retention, bounded scope/stage/outcome labels, omission categories, redaction counters, safe diagnostics, all-false authority, raw-data prohibitions, threat boundaries, and persistence/upload/release non-goals. Run `npm run validate:contracts && npm run validate:docs && npm run check && git diff --check` after changing the schema, fixtures, or ADR.
- `architecture/040-agent-execution-and-architecture-discipline.md` — accepted repository execution discipline: canonical pre-edit reading matrix, subsystem ownership, task/card traceability, implementation-versus-evidence vocabulary, verification tiers, review/fix gates, and public-metadata safety. Its validator is included in `npm run check`; focused validation is `npm run check:agent-architecture-contract`.
- `architecture/041-current-capability-truth.md` — accepted canonical current capability truth: exact provenance vocabulary, evidence-backed major-surface rows, limitations, and Browser/VS Code/JetBrains host matrix. Read it before product or capability work; the architecture validator enforces its durable links, required rows, and statuses.
- S125-S135 controlled workflow hardening convergence commands are wired as root scripts. `npm run smoke:vscode-installable` and `npm run smoke:vscode-packaged-controlled-task` remain intentionally separate artifact-dependent VS Code checks: the installable smoke answers the generic VSIX/archive layout, identity, checksum, GUI asset, and bundled-engine install-from-file readiness question, while the packaged controlled-task smoke answers the S129 controlled-task evidence-boundary question for explicit gates, sanitized report categories, reconnect/reload/stale-result guidance, and unsupported or fail-closed host wording. Fast deterministic metadata gates are available as `npm run smoke:controlled-agent-task-harness`, `npm run smoke:controlled-agent-workflow-transcript`, and `npm run check:controlled-agent-storage-privacy`; only the dependency-light storage/privacy checker is included in `npm run check`. The GUI TypeScript/transpile-dependent gates remain explicit because isolated worktrees can lack `apps/gui/node_modules` until local setup is restored. Explicit heavier, GUI-dependent, VS Code/plugin-dependent, artifact-dependent, or final convergence gates remain separate: `npm run verify:controlled-agent-gui`, `npm run verify:controlled-agent-vscode`, `npm run verify:controlled-agent-packaged`, `npm run verify:controlled-agent-repo-hygiene`, and `npm run verify:controlled-agent-hardening-final`. These commands are dev-preview/local evidence only: no provider calls, hosted Yet AI backend, account, managed gateway, product credit, cloud workspace, VS Code UI launch, package publication, signing, notarization, marketplace approval, production release, automatic apply, automatic verification, or broader autonomy claim is made.

S131-C1 classifies those S125-S130 commands in `architecture/003-target-architecture.md` as lightweight/root-safe, GUI-dependent, VS Code/plugin-dependent, artifact-dependent, or final convergence only. S131-C2 adds filter-safe wrappers for the GUI chain (`npm run verify:controlled-agent-gui`), VS Code/plugin chain (`npm run verify:controlled-agent-vscode`), and repo hygiene (`npm run verify:controlled-agent-repo-hygiene`) so those evidence lanes can be reported without ambiguous `&&` chains. S131-C3 keeps `npm run check` on the lightweight/root-safe side: it no longer invokes GUI TypeScript/transpile-dependent controlled-agent smokes directly, while their explicit `npm run smoke:*` commands and wrapper/bundle commands remain available for focused verification after GUI dependency setup. S135 adds the packaged wrapper (`npm run verify:controlled-agent-packaged`) and the final wrapper (`npm run verify:controlled-agent-hardening-final`), which runs the lightweight check, GUI wrapper, VS Code wrapper, task harness smoke, workflow transcript smoke, storage/privacy checker, packaged prepare/archive wrapper, and repo hygiene wrapper in order with bounded sanitized output.

S130 final verification/postmortem and S131-S135 wrapper convergence are recorded in `architecture/003-target-architecture.md`. The final local pass included agent-reported local runs for contracts, storage/privacy, task harness, workflow transcript, built-GUI controlled-task smoke, prepared local VSIX, packaged VS Code controlled-task smoke, hardening bundle, `npm run check`, and diff/status hygiene; later wrapper evidence covers GUI, VS Code/plugin, packaged prep/archive inspection, repo hygiene, and final hardening through `npm run verify:controlled-agent-gui`, `npm run verify:controlled-agent-vscode`, `npm run verify:controlled-agent-packaged`, `npm run verify:controlled-agent-repo-hygiene`, and `npm run verify:controlled-agent-hardening-final`. Treat this as conservative local/mock/dev-preview and archive-inspection evidence, not verifier-backed proof for every older rejected or chained command group. Manual BYOK dogfood, real IDE install-from-file launch/reconnect/reload/update recovery, real-provider evidence, signing, notarization, release, support readiness, and marketplace work remain separate decisions.
...
- The experimental Codex-like provider-auth path is high-risk, non-default, manual/dev-preview, and mock-only in automation.
- Experimental first-message recovery copy is manual-only and sanitized: retry login, reconnect runtime, disconnect, reduce context when relevant, or use the API-key fallback; it does not imply automatic retry, official OAuth, production login support, or managed support.
...
| Verification commands | `unsupported`; preview-only/non-executing | `live_host` dev-preview for explicit fixed allowlisted controlled command and verification-bundle requests; no free-form shell/args/cwd/env/git/network | `unsupported`; controlled verification envelopes fail closed |
| Controlled runner | `local_derived` GUI state only; no trusted execution | `local_derived` orchestration over explicit `live_host` bounded executors; no background or multi-step autonomy | `local_derived` GUI state with controlled executors `unsupported`/fail-closed |
...
## Controlled capability provenance

Use ADR 041's exact statuses when describing controlled-agent surfaces. The VS Code bounded read, lexical-search, replacement-edit, multi-file apply, allowlisted command-run, and verification-bundle executors are `live_host` dev-preview behavior. GUI readiness, run-state, progress-report, and recovery calculations are `local_derived`. Contract examples, mock `/v1/caps` fields, Demo Mode, and harness-only workflow data are `fixture_demo`; the real engine does not currently emit the full fixture-rich controlled caps payload. Browser controlled execution and JetBrains controlled read/search/edit/multi-file/verification envelopes are `unsupported` and fail closed, even when their GUI can render labeled preview metadata. Agent-progress endpoints are `live_engine`, but lifecycle population remains `local_derived` through scripts, smokes, or explicit developer calls rather than automatic controlled-run publication.

A smoke proves only the boundary it actually drives. Pure-service, fixture, built-GUI mock, wrapper-browser, and archive-inspection smokes do not become installed-host execution evidence. Controlled UI actions require matching `live_host` provenance in addition to the existing readiness, request-minting, correlation, policy, and confirmation gates.

## Controlled lexical search smoke

For S110 controlled lexical search evidence, run `npm run smoke:controlled-agent-lexical-search` from the repository root. It is a fast deterministic local/mock smoke over the S110 contract fixtures and `apps/gui/src/services/controlledAgentLexicalSearch.ts`; it verifies a safe explicit VS Code request summary, Browser unsupported, JetBrains fail-closed, unsafe queries, hidden indexing/background-search claims, unsafe snippets/private paths/secrets/raw payload markers, and no automatic context attachment, provider/tool call, apply, or verification authority. This is contract and pure-GUI-service evidence only, not real VS Code search execution.

For S111 VS Code executor evidence, run `npm run smoke:controlled-agent-real-lexical-search` and `cd apps/plugins/vscode && npm run compile && npm test`. The focused real-executor smoke compiles/imports the VS Code plugin executor and uses a disposable sentinel workspace to verify bounded literal snippets and fail-closed unsafe cases without launching VS Code UI, providers, runtime services, network, git, package installation, background indexing, broad workspace scans, shell/tool execution, or file mutation.
...
The historical S84 slice intentionally disabled the older `gui.ideActionRequest` verification action. The current controlled path is the later S85 `gui.controlledAgentCommandRunRequest` and S117 verification-bundle path in VS Code; those fixed-id executors are `live_host` dev-preview behavior and do not revive the older request family or broaden authority.
...
For S90 controlled-autonomy readiness, use `docs/dogfood/s90-controlled-autonomy-readiness.md`. The matrix records evidence and `ready`, `partial`, or `blocked` decisions for explicit Start/Stop, bounded read, bounded edit, allowlisted verification, one repair attempt, dogfood usefulness, cross-host availability, resilience, and public wording safety. The overall S90 decision is `partial`: approve only an experimental controlled local agent dev-preview for narrow local/mock and explicit-user-start evidence. It is useful for narrow dev-preview local dogfood planning, but not production autonomy, not broad workspace automation, not real-provider CI, not release or marketplace evidence, and not cross-host complete. Browser remains preview-only/unsupported for trusted workspace execution, JetBrains remains fail-closed for controlled execution parity, and VS Code is the only current real controlled execution host for implemented slices. The exact S90 audit chain is `npm run smoke:controlled-autonomy-readiness && npm run audit:controlled-autonomy-wording && npm run check`; the readiness smoke runs the S86 one-step loop, S87 one-attempt repair loop, S88 useful dogfood fixture gate, S90 public wording audit, and S89 resilience when available.

For S91-S94 controlled local agent dev-preview alignment, use `docs/dogfood/s91-controlled-agent-dev-preview.md`. The document carries the S90 `partial` decision forward into a VS Code-first dev-preview target: explicit Start/Stop, bounded read/edit/allowlisted-verification, at most one user-confirmed repair attempt, and sanitized progress/final reporting. Browser remains preview-only/unsupported for trusted workspace execution, JetBrains remains partial/fail-closed for controlled execution parity, and S91-S94 do not add production autonomy, real-provider CI, release or marketplace evidence, hidden reads/search/indexing, arbitrary shell, broad mutation, automatic retry/rollback, raw payload persistence, or cross-host completion. The final S94 documentation closure gate is `npm run audit:controlled-autonomy-wording`, `npm run check`, and `git diff --check && git status --short` before sharing.

For S96 one-step useful-run documentation, keep using `docs/dogfood/s91-controlled-agent-dev-preview.md` plus `architecture/013-agent-readiness-milestone.md`. S96 is a docs contract for a VS Code-first controlled dev-preview target: explicit **Start** → one bounded controlled read → one bounded replacement edit → one allowlisted verification command id → one sanitized terminal report. It preserves the same non-goals: no production autonomy, hidden reads/search/indexing, arbitrary shell, multi-step autonomy, automatic repair, automatic retry, automatic rollback, raw payload persistence, browser trusted workspace execution, JetBrains controlled execution parity, real-provider CI, release evidence, or marketplace evidence. The S96 documentation gate is:
...
The smoke transpiles and imports the pure GUI services locally, drafts a one-step safe-edit prompt from explicit attached context only, feeds mock assistant safe-edit and controlled provider-proposal responses into proposal correlation, and evaluates Agent Run readiness metadata. It covers both a valid strict safe-edit proposal and malformed/unsafe proposal rejection, plus a metadata-only mock provider proposal that must stay sanitized, create no tool calls, create no apply or verification request, reject raw payload authority, and avoid raw provider payload persistence. It is deterministic local/mock evidence only: it does not launch an IDE, call providers, perform network requests, execute shell/git/tools, scan hidden workspace files, write browser storage, auto-apply edits, or auto-run verification.
...
`npm run check` runs the repository's local validation bundle: product identity, public hygiene, docs index coverage, IDE surface contract parity, docs validation, contracts, root-safe metadata validators, and dogfood/report template self-tests that are safe for the current checkout. It does not run GUI TypeScript/transpile smokes, GUI builds, VS Code/plugin compile or tests, Playwright/browser smoke gates, packaged IDE smoke gates, provider calls, hosted Yet AI services, marketplace publication, or production release checks. Run focused smoke and wrapper commands explicitly when changing those heavier surfaces after restoring the relevant local dependencies.
...
Use `docs/dogfood/controlled-agent-real-provider-matrix.md` for repeatable manual local BYOK controlled-agent evidence across provider/runtime families, failure paths, and the five controlled task presets. Copy the tracked v2 template to an ignored local evidence location, assign a safe run-group label and distinct repeated-run labels, predeclare a bounded repeat count, keep comparable scenario/context/host/artifact/verification conditions fixed where practical, and record every attempt independently. Each attempt records safe provider/runtime family, host surface, artifact label, explicit context/proposal/apply/verification outcomes, failure/recovery path, reconnect/reload/stale outcome, and sanitized final assessment; all unobserved and tracked-template fields stay `not run`. Generate or validate the tracked template with `npm run dogfood:controlled-agent-real-provider-matrix -- --template` and `npm run dogfood:controlled-agent-real-provider-matrix -- --check-template`; validate completed ignored local evidence with `npm run dogfood:controlled-agent-real-provider-matrix -- --check path/to/local-matrix.md`; run `npm run dogfood:controlled-agent-real-provider-matrix -- --self-test` for sanitizer coverage. The checker is manual evidence support only: it does not call providers, launch runtimes, automate CI real-provider runs, require hosted Yet AI services, or prove production, release, marketplace, publication, account, gateway, credit, cloud-workspace, or autonomy readiness. Browser remains preview-only/unsupported for trusted workspace execution and JetBrains remains partial/fail-closed. The contract forbids credentials, account identifiers, raw prompts/responses/provider payloads, file/diff/replacement bodies, raw command/output material, private paths, bridge dumps, hosted requirements, real-provider CI claims, and production/release/marketplace claims.

Use `docs/dogfood/experimental-codex-login.md` for explicit manual real-account dogfood of the high-risk experimental Codex-like login path. Generate or validate the tracked template with `npm run dogfood:experimental-codex-login-report -- --template` and `npm run dogfood:experimental-codex-login-report -- --check-template`; validate completed ignored local evidence with `npm run dogfood:experimental-codex-login-report -- --check path/to/local-report.md`; run `npm run dogfood:experimental-codex-login-report -- --self-test` for sanitizer coverage. The checker is manual evidence support only: it does not call providers, launch runtimes, automate real-provider CI, store secrets, post bridge messages, mutate workspaces, sign, publish, or prove official OAuth, production login, release, marketplace, signing, notarization, support, hosted-service, account, gateway, credit, cloud-workspace, or CI readiness. It rejects API keys, bearer or authorization headers, access/refresh tokens, auth codes, PKCE verifiers, cookies, secret URL query or fragment values, raw account ids, token claims, raw provider responses, raw prompts, raw file bodies, raw diffs, private absolute paths, bridge dumps, browser-storage dumps, hosted requirements, automation claims, and official/readiness overclaims.
