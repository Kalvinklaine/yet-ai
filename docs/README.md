...
- S125-S130 controlled workflow hardening convergence commands are wired as root scripts. `npm run smoke:vscode-installable` and `npm run smoke:vscode-packaged-controlled-task` are intentionally separate artifact-dependent VS Code checks: the installable smoke answers the generic VSIX/archive layout, identity, checksum, GUI asset, and bundled-engine install-from-file readiness question, while the packaged controlled-task smoke answers the S129 controlled-task evidence-boundary question for explicit gates, sanitized report categories, reconnect/reload/stale-result guidance, and unsupported or fail-closed host wording. Fast deterministic metadata gates are available as `npm run smoke:controlled-agent-task-harness`, `npm run smoke:controlled-agent-workflow-transcript`, and `npm run check:controlled-agent-storage-privacy`; only the dependency-light storage/privacy checker is included in `npm run check`. The GUI TypeScript/transpile-dependent gates remain explicit because isolated worktrees can lack `apps/gui/node_modules` until local setup is restored. Explicit heavier, GUI-dependent, or artifact-dependent gates remain separate: `npm run smoke:controlled-agent-task-harness`, `npm run smoke:controlled-agent-workflow-transcript`, `npm run smoke:controlled-agent-search-selection`, `npm run smoke:controlled-agent-task-presets`, `npm run smoke:controlled-agent-patch-plan-preview`, `npm run smoke:controlled-agent-recovery-matrix`, `npm run smoke:controlled-agent-verification-followup`, `npm run smoke:controlled-run-observability`, `npm run smoke:controlled-run-history`, `npm run smoke:model-proposal-agent-run`, `npm run smoke:controlled-agent-vscode-task-e2e` requires a current built GUI, `npm run smoke:vscode-packaged-controlled-task` requires a prepared local dev-preview VSIX from `npm run prepare:vscode-preview`, and `npm run smoke:controlled-agent-hardening-bundle` fail-fast chains the S125/S126/S127/S129 gates plus selected existing controlled-agent local/mock gates. These commands are dev-preview/local evidence only: no provider calls, hosted Yet AI backend, account, managed gateway, product credit, cloud workspace, VS Code launch, package publication, signing, notarization, marketplace approval, production release, automatic apply, automatic verification, or broader autonomy claim is made.

S131-C1 classifies those S125-S130 commands in `architecture/003-target-architecture.md` as lightweight/root-safe, GUI-dependent, VS Code/plugin-dependent, artifact-dependent, or final convergence only. S131-C2 adds filter-safe wrappers for the GUI chain (`npm run verify:controlled-agent-gui`), VS Code/plugin chain (`npm run verify:controlled-agent-vscode`), and repo hygiene (`npm run verify:controlled-agent-repo-hygiene`) so those evidence lanes can be reported without ambiguous `&&` chains. Packaged prepare/smoke evidence and the hardening bundle remain explicit commands unless a later wrapper card adds dedicated wrapper scripts. S131-C3 keeps `npm run check` on the lightweight/root-safe side: it no longer invokes GUI TypeScript/transpile-dependent controlled-agent smokes directly, while their explicit `npm run smoke:*` commands and wrapper/bundle commands remain available for focused verification after GUI dependency setup.

S130 final verification/postmortem is recorded in `architecture/003-target-architecture.md`. The final local pass included agent-reported local runs for contracts, storage/privacy, task harness, workflow transcript, built-GUI controlled-task smoke, prepared local VSIX, packaged VS Code controlled-task smoke, hardening bundle, `npm run check`, and diff/status hygiene; later S131 wrapper evidence covers the GUI chain, VS Code/plugin chain, and repo hygiene through `npm run verify:controlled-agent-gui`, `npm run verify:controlled-agent-vscode`, and `npm run verify:controlled-agent-repo-hygiene`. Treat S130 as conservative local/mock dev-preview evidence, not verifier-backed proof for every rejected or chained command group. Manual BYOK dogfood, real IDE launch/install/reconnect recovery evidence, real-provider evidence, signing, notarization, release, and marketplace work remain separate decisions.
