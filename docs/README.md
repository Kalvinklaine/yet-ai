...
- `architecture/019-controlled-agent-explicit-lexical-search.md` — S110 explicit lexical search contract for GUI/user-minted, VS Code-only, bounded literal search metadata with sanitized snippet results, Browser unsupported, JetBrains fail-closed, and no hidden indexing, broad scan, regex/glob, provider/tool, automatic search, implementation, release, or marketplace authority.
