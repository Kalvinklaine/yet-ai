...
Use `docs/dogfood/controlled-agent-real-provider-matrix.md` for manual local BYOK controlled-agent real-provider matrix evidence across provider/runtime families and the five controlled task presets. Generate or validate the tracked template with `npm run dogfood:controlled-agent-real-provider-matrix -- --template` and `npm run dogfood:controlled-agent-real-provider-matrix -- --check-template`; validate completed ignored local evidence with `npm run dogfood:controlled-agent-real-provider-matrix -- --check path/to/local-matrix.md`; run `npm run dogfood:controlled-agent-real-provider-matrix -- --self-test` for sanitizer coverage. The checker is manual evidence support only: it does not call providers, launch runtimes, automate CI real-provider runs, require hosted Yet AI services, or prove production, release, marketplace, publication, account, gateway, credit, cloud-workspace, or autonomy readiness. It rejects secrets, raw prompts/responses/file bodies/diffs/replacements, raw command/stdout/cwd/env material, private paths, provider payloads, bridge payload dumps, hosted requirements, real-provider CI claims, and production/release/marketplace claims.
