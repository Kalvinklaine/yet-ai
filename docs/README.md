...
- `architecture/034-experimental-codex-login-dogfood-audit.md` — S136-C1 current-flow audit and dogfood gap matrix for the experimental Codex-like login path. It maps start/status/exchange/disconnect, engine secret custody and refresh behavior, GUI and VS Code visible flow, mock-only versus real/manual boundaries, reference-derived UX requirements, severity-ranked gaps, and S141-S142 decision gates while preserving experimental, private-endpoint-style, non-default, not-official-OAuth, and not-production-ready wording. Run `npm run check` and `git diff --check` after changing this audit.
