...
- `architecture/029-vscode-controlled-task-harness.md` — S125 VS Code-first controlled task harness contract for one deterministic dogfood hardening journey. It defines metadata-only task preset, explicit context/search selection, proposal review, patch-plan review, explicit apply, verification bundle, follow-up/recovery, stale or duplicate correlation fail-closed states, unsupported Browser, partial/fail-closed JetBrains, and no production/release/marketplace claims. Run `npm run validate:contracts` and `npm run check` after changing the S125-C1 schema or fixtures.
