...
- `architecture/019-controlled-agent-explicit-lexical-search.md` — S110 explicit lexical search contract for GUI/user-minted, VS Code-only, bounded literal search metadata with sanitized snippet results, Browser unsupported, JetBrains fail-closed, and no hidden indexing, broad scan, regex/glob, provider/tool, automatic search, implementation, release, or marketplace authority. Run `npm run smoke:controlled-agent-lexical-search` after changing the S110 lexical-search contract fixtures or the GUI pure service; it is deterministic local/mock evidence only and is included in `npm run check` because it performs no host execution, workspace search, provider/tool call, apply, verification, network, shell, git, or file mutation.
