...
- `architecture/028-useful-multifile-controlled-agent-decision.md` — S124-C1 useful multi-file controlled agent evidence decision summary. It classifies S109-S123 authority registry, explicit search/selection, provider proposal, multi-file patch review/apply, verification/follow-up, two-step state, recovery, task presets, real-provider dogfood, and packaged beta gate evidence as pass/partial/blocked, preserves non-goals, and recommends hardening before broader autonomy or release work. Run `npm run check` and `git diff --check` after changing this decision record.
