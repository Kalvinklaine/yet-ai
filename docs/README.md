...
For S90 controlled-autonomy readiness, use `docs/dogfood/s90-controlled-autonomy-readiness.md`. The matrix records evidence and `ready`, `partial`, or `blocked` decisions for explicit Start/Stop, bounded read, bounded edit, allowlisted verification, one repair attempt, dogfood usefulness, cross-host availability, resilience, and public wording safety. The overall S90 decision is `partial`: useful for narrow dev-preview local dogfood planning, but not production autonomy, not real-provider CI, not broad workspace authority, and not cross-host complete. Browser remains preview-only/unsupported for trusted workspace execution, JetBrains remains fail-closed for controlled execution parity, and VS Code is the only current real controlled execution host for implemented slices. For S90 documentation-only updates, run `npm run check`.
