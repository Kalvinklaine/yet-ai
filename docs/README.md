...
S131-C1 classifies those S125-S130 commands in `architecture/003-target-architecture.md` as lightweight/root-safe, GUI-dependent, VS Code/plugin-dependent, artifact-dependent, or final convergence only. S131-C2 wrapper work is needed for GUI chains, VS Code/plugin chains, repo hygiene checks, packaged prepare/smoke evidence, and final bundle/status commands so filtered or failed `&&` command groups cannot be reported as complete evidence. S131-C3 keeps `npm run check` on the lightweight/root-safe side: it no longer invokes GUI TypeScript/transpile-dependent controlled-agent smokes directly, while their explicit `npm run smoke:*` commands and wrapper/bundle commands remain available for focused verification after GUI dependency setup.

S130 final verification/postmortem is recorded in `architecture/003-target-architecture.md`. The final local pass included contracts, GUI typecheck/focused tests/build, VS Code compile/tests, storage/privacy, task harness, workflow transcript, built-GUI controlled-task smoke, prepared local VSIX, packaged VS Code controlled-task smoke, hardening bundle, `npm run check`, and diff/status hygiene. The status remains dogfood-ready only for conservative local/mock dev-preview evidence; manual BYOK dogfood, real IDE launch/install recovery evidence, real-provider evidence, signing, notarization, release, and marketplace work remain separate decisions.
