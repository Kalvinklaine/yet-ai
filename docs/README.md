...
- `architecture/021-controlled-agent-multifile-patch-plan.md` — S114 bounded multi-file patch plan contract for review-only existing-file replacement metadata. It records file/edit/replacement byte budgets, expected pre-edit hashes/ranges, per-file summaries and risk labels, while forbidding raw replacement bodies, raw diffs, create/delete/rename/move/chmod/binary/symlink/dependency/generated/hidden/private-path edits, assistant-minted apply, model-minted apply authority, command/provider/tool authority, and runtime/host apply implementation. Run `npm run validate:contracts` after changing the S114 schema or fixtures.
- `architecture/022-controlled-agent-verification-bundles.md` — S117 verification bundle v1 contract for explicit user-approved bounded sequences of fixed allowlisted command ids. It records controlled workspace/run correlation, per-command timeout/output-tail budgets, sanitized aggregate summaries, and local/mock deterministic fixture evidence while forbidding free-form command strings, args/cwd/env/shell, git/package/network/provider/tool fields, raw output dumps, private paths, secrets, automatic run claims, production/release overclaims, and any execution implementation. Run `npm run validate:contracts` after changing the S117 schema or fixtures.
