...
- S125-S130 controlled workflow hardening convergence commands are wired as root scripts. Fast deterministic metadata gates are available as `npm run smoke:controlled-agent-task-harness`, `npm run smoke:controlled-agent-workflow-transcript`, and `npm run check:controlled-agent-storage-privacy`; these are included in `npm run check` where safe. Explicit heavier or artifact-dependent gates remain separate: `npm run smoke:controlled-agent-vscode-task-e2e` requires a current built GUI, `npm run smoke:vscode-packaged-controlled-task` requires a prepared local dev-preview VSIX from `npm run prepare:vscode-preview`, and `npm run smoke:controlled-agent-hardening-bundle` fail-fast chains the S125/S126/S127/S129 gates plus selected existing controlled-agent local/mock gates. These commands are dev-preview/local evidence only: no provider calls, hosted Yet AI backend, account, managed gateway, product credit, cloud workspace, VS Code launch, package publication, signing, notarization, marketplace approval, production release, automatic apply, automatic verification, or broader autonomy claim is made.

S131-C1 classifies those S125-S130 commands in `architecture/003-target-architecture.md` as lightweight/root-safe, GUI-dependent, VS Code/plugin-dependent, artifact-dependent, or final convergence only. S131-C2 wrapper work is needed for GUI chains, VS Code/plugin chains, repo hygiene checks, packaged prepare/smoke evidence, and final bundle/status commands so filtered or failed `&&` command groups cannot be reported as complete evidence. This classification does not add anything to `npm run check`.
