...
- `architecture/036-browser-project-isolation.md` — authoritative architecture for browser-first project isolation v1: opaque project identity, private canonical-root registry, `/projects` hub and scoped route contracts, request-scoped `ProjectContext`, config/cache namespaces, bounded directory discovery, migration/rollback, strict switching, safe hub fields, and explicit non-goals. This is a decision record, not an implementation claim. Run `npm run validate:docs`, `npm run validate:hygiene`, and `git diff --check` after changing it.
