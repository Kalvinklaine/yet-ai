...
For S91 controlled local agent dev-preview alignment, use `docs/dogfood/s91-controlled-agent-dev-preview.md`. The document carries the S90 `partial` decision forward into a VS Code-first dev-preview target: explicit Start/Stop, bounded read/edit/allowlisted-verification, at most one user-confirmed repair attempt, and sanitized progress/final reporting. Browser remains preview-only/unsupported for trusted workspace execution, JetBrains remains fail-closed/unsupported for controlled execution parity, and S91 does not add production autonomy, real-provider CI, release or marketplace evidence, hidden reads/search/indexing, arbitrary shell, broad mutation, automatic retry/rollback, or raw payload persistence. The documentation gate is `npm run audit:controlled-autonomy-wording && npm run check`, followed by `git diff --check && git status --short` before sharing.
